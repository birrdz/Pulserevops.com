// SEO-optimize all live cg#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _cg_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const cgRows = (idx.entries || []).filter((e) => e && /^cg\d+$/i.test(e.id));
  cgRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of cgRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: CG_KEYWORD_PHRASES.length,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(CG_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: CG_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: CG_STATIC_URLS,
  };
  fs.writeFileSync('C:/Users/koryj/website/_cg_seo_optimize_report.json', JSON.stringify(report, null, 1));
