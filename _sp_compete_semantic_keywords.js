// Pulse Speeches — semantic search + FAQ compete keywords for sp#### entries.
const SP_KEYWORD_PHRASES = require('./_sp_keyword_phrases.json');

const SP_HUB_URL = 'https://pulserevops.com/speeches';
const SP_HUB_PAGE = 'speeches.html';
const SP_STATIC_URLS = [SP_HUB_URL];
...
function phraseToFaq(phrase) {
  ...
  const a = `Pulse Speeches ranks ${phrase} with Best Overall and Best Value picks and a buyer decision tree for 2027.`;
  return { q, a };
}

const SEMANTIC_FAQ_HUB_SCHEMA = SP_KEYWORD_PHRASES.slice(0, 48).map(phraseToFaq);

const COMPETE_DISPLAY_KEYWORDS = [
  'Semantic Search: Pulse Speeches FAQ',
  'Pulse Speeches FAQ structured data',
  'Pulse Speeches semantic search optimization',
  'Pulse Speeches Top-10 Rankings',
  'Best Overall vs Best Value',
  'top-10 rankings 2027',
  ...SP_KEYWORD_PHRASES,
];

const GLOBAL_TAGS = ["pulse-speeches","speeches","toasts","top-10","best-of-2027"];

function keywordsFromTitle(title) {
  const t = String(title || '').toLowerCase();
  const hits = [];
  for (const phrase of SP_KEYWORD_PHRASES) {
  ...
  if (!hits.length) hits.push(...SP_KEYWORD_PHRASES.slice(0, 8).map(slugify));
  return hits;
}
...
function tagsForEntry(id, question, existingTags = []) {
  ...
  const allSlugs = SP_KEYWORD_PHRASES.map(slugify);
...
function hubKeywordItemListJsonLd(hubUrl) {
  return {
    ...
    name: 'Pulse Speeches Search Index',
    numberOfItems: SP_KEYWORD_PHRASES.length,
    itemListElement: SP_KEYWORD_PHRASES.map((phrase, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: phrase,
...
function entrySeoBrandKeywords(question) {
  return Array.from(new Set([
    ...
    'Pulse Speeches review',
    'Pulse Speeches reviews',
    'Pulse Speeches rankings',
  ]));
}
