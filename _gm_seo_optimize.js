// SEO-optimize all live gm#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _gm_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const gmRows = (idx.entries || []).filter((e) => e && /^gm\d+$/i.test(e.id));
  gmRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of gmRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: GM_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/gaming/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(GM_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: GM_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: GM_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_gm_seo_optimize_report.json', JSON.stringify(report, null, 1));
