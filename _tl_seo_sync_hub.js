// Sync Pulse Tools hub meta + JSON-LD. Usage: node _tl_seo_sync_hub.js
const fs = require('fs');
const path = require('path');
const { TL_HUB_PAGE, TL_HUB_URL, hubMetaKeywords, hubFaqJsonLd, hubKeywordItemListJsonLd, TL_KEYWORD_PHRASES } = require('./_tl_compete_semantic_keywords');
const { pingIndexNowUrlList } = require('./netlify/functions/lib/indexnow-ping-entry');

const HUB = path.join(__dirname, TL_HUB_PAGE);
...
const collectionLd = {
  '@context': 'https://schema.org', '@type': 'CollectionPage', '@id': `${TL_HUB_URL}#webpage`,
  name: 'Pulse Tools', url: TL_HUB_URL, description: "Top-10 calculators and operator tools for revenue teams.",
  keywords: phrases.slice(0, 24).join(', '),
};
const ldBlock = `<script type="application/ld+json">\n${JSON.stringify(collectionLd, null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubFaqJsonLd(), null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubKeywordItemListJsonLd(TL_HUB_URL), null, 2)}\n</script>`;
...
console.log('hub synced', TL_HUB_PAGE, 'phrases', TL_KEYWORD_PHRASES.length);
(async () => { const ping = await pingIndexNowUrlList([TL_HUB_URL]); console.log('hub IndexNow', JSON.stringify(ping)); })().catch(console.error);
