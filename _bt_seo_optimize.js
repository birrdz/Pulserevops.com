// SEO-optimize all live bt#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _bt_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const btRows = (idx.entries || []).filter((e) => e && /^bt\d+$/i.test(e.id));
  btRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of btRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: BT_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/boats/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(BT_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: BT_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: BT_STATIC_URLS,
  };
  fs.writeFileSync('C:/Users/koryj/website/_bt_seo_optimize_report.json', JSON.stringify(report, null, 1));
