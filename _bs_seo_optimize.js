// SEO-optimize all live bs#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _bs_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const bsRows = (idx.entries || []).filter((e) => e && /^bs\d+$/i.test(e.id));
  bsRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of bsRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: BS_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/sales-book-summaries/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(BS_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: BS_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: BS_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_bs_seo_optimize_report.json', JSON.stringify(report, null, 1));
