...
//
// Usage: node _write_sy.js <sy####> "<title>" [topic-slug]
...
if (!ID || !/^sy\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_sy.js <sy####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['style', 'work-style', 'business-attire', 'workwear'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/style/${ID}`, indexnow: indexed }));
