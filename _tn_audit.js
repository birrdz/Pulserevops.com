...
const tns=(idx.entries||[]).filter(e=>e&&/^tn\d+$/.test(e.id)).sort((a,b)=>a.id.localeCompare(b.id));
let ok=true;
for(const e of tns){const rec=await s.get('answers/'+e.id+'.json',{type:'json'});const b=rec.answer;const g=gradeEntry(e.id,b);const iss=[];
if(g.score<12)iss.push('score '+g.score+'/12['+g.missing.join(',')+']');if(g.word_count<1900)iss.push('words '+g.word_count);if(g.banned_hits&&g.banned_hits.length)iss.push('banned');
...
if(e.id==='tn0002')fs.writeFileSync('C:/Users/koryj/_tn0002_view.md',b);}
