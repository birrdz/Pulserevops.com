...
const sys=(idx.entries||[]).filter(e=>e&&/^sy\d+$/.test(e.id)).sort((a,b)=>a.id.localeCompare(b.id));
let allClean=true;
for(const e of sys){const rec=await s.get('answers/'+e.id+'.json',{type:'json'});const b=rec.answer;const g=gradeEntry(e.id,b);const iss=[];
const isTop10=/^top 10/i.test(e.question);
if(g.score<11)iss.push('score '+g.score+'/12 ['+g.missing.join(',')+']');
if(g.banned_hits&&g.banned_hits.length)iss.push('banned['+g.banned_hits.join(',')+']');
if(iss.length)allClean=false;
console.log((iss.length?'FAIL':'PASS')+' ['+(isTop10?'T10':'Q&A')+'] '+e.id+' ('+g.word_count+'w) — '+e.question+(iss.length?(' :: '+iss.join(' | ')):''));
if(e.id==='sy0001'||e.id==='sy0009')fs.writeFileSync('C:/Users/koryj/_'+e.id+'_view.md',b);}
