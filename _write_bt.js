// Direct-write for the boats pillar — bt####.
...
//
// Usage: node _write_bt.js <bt####> "<title>" [topic-slug]
...
if (!ID || !/^bt\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_bt.js <bt####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['boat', 'top-10', 'best-of-2027', 'boats'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/boats/${ID}`, indexnow: indexed }));
