// Pulse Travel — semantic search + FAQ compete keywords for tv#### entries.
const TV_KEYWORD_PHRASES = require('./_tv_keyword_phrases.json');

const TV_HUB_URL = 'https://pulserevops.com/travel';
const TV_HUB_PAGE = 'travel.html';
const TV_STATIC_URLS = [TV_HUB_URL];
...
function phraseToFaq(phrase) {
  ...
  const a = `Pulse Travel ranks ${phrase} with Best Overall and Best Value picks and a buyer decision tree for 2027.`;
  return { q, a };
}

const SEMANTIC_FAQ_HUB_SCHEMA = TV_KEYWORD_PHRASES.slice(0, 48).map(phraseToFaq);

const COMPETE_DISPLAY_KEYWORDS = [
  'Semantic Search: Pulse Travel FAQ',
  'Pulse Travel FAQ structured data',
  'Pulse Travel semantic search optimization',
  'Pulse Travel Top-10 Rankings',
  'Best Overall vs Best Value',
  'top-10 rankings 2027',
  ...TV_KEYWORD_PHRASES,
];

const GLOBAL_TAGS = ["pulse-travel","travel","destinations","top-10","best-of-2027"];

function keywordsFromTitle(title) {
  const t = String(title || '').toLowerCase();
  const hits = [];
  for (const phrase of TV_KEYWORD_PHRASES) {
  ...
  if (!hits.length) hits.push(...TV_KEYWORD_PHRASES.slice(0, 8).map(slugify));
  return hits;
}
...
function tagsForEntry(id, question, existingTags = []) {
  ...
  const allSlugs = TV_KEYWORD_PHRASES.map(slugify);
...
function hubKeywordItemListJsonLd(hubUrl) {
  return {
    ...
    name: 'Pulse Travel Search Index',
    numberOfItems: TV_KEYWORD_PHRASES.length,
    itemListElement: TV_KEYWORD_PHRASES.map((phrase, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: phrase,
...
function entrySeoBrandKeywords(question) {
  return Array.from(new Set([
    ...
    'Pulse Travel review',
    'Pulse Travel reviews',
    'Pulse Travel rankings',
  ]));
}
