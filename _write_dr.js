...
//
// Usage: node _write_dr.js <dn####> "<title>" [topic-slug]
...
if (!ID || !/^dr\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_dr.js <dr####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['drill', 'skill-drill', 'workshop', 'training'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/drills/${ID}`, indexnow: indexed }));
