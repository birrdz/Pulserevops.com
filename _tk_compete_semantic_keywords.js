// Pulse Tech Stacks — semantic search + FAQ compete keywords for tk#### entries.
const TK_KEYWORD_PHRASES = require('./_tk_keyword_phrases.json');

const TK_HUB_URL = 'https://pulserevops.com/tech-stacks';
const TK_HUB_PAGE = 'tech-stacks.html';
const TK_STATIC_URLS = [TK_HUB_URL];
...
function phraseToFaq(phrase) {
  ...
  const a = `Pulse Tech Stacks ranks ${phrase} with Best Overall and Best Value picks and a buyer decision tree for 2027.`;
  return { q, a };
}

const SEMANTIC_FAQ_HUB_SCHEMA = TK_KEYWORD_PHRASES.slice(0, 48).map(phraseToFaq);

const COMPETE_DISPLAY_KEYWORDS = [
  'Semantic Search: Pulse Tech Stacks FAQ',
  'Pulse Tech Stacks FAQ structured data',
  'Pulse Tech Stacks semantic search optimization',
  'Pulse Tech Stacks Top-10 Rankings',
  'Best Overall vs Best Value',
  'top-10 rankings 2027',
  ...TK_KEYWORD_PHRASES,
];

const GLOBAL_TAGS = ["pulse-tech-stacks","tech-stacks","software-stack","top-10","best-of-2027"];

function keywordsFromTitle(title) {
  const t = String(title || '').toLowerCase();
  const hits = [];
  for (const phrase of TK_KEYWORD_PHRASES) {
  ...
  if (!hits.length) hits.push(...TK_KEYWORD_PHRASES.slice(0, 8).map(slugify));
  return hits;
}
...
function tagsForEntry(id, question, existingTags = []) {
  ...
  const allSlugs = TK_KEYWORD_PHRASES.map(slugify);
...
function hubKeywordItemListJsonLd(hubUrl) {
  return {
    ...
    name: 'Pulse Tech Stacks Search Index',
    numberOfItems: TK_KEYWORD_PHRASES.length,
    itemListElement: TK_KEYWORD_PHRASES.map((phrase, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: phrase,
...
function entrySeoBrandKeywords(question) {
  return Array.from(new Set([
    ...
    'Pulse Tech Stacks review',
    'Pulse Tech Stacks reviews',
    'Pulse Tech Stacks rankings',
  ]));
}
