// Direct-write for the Chief-keyword line — Q&A entries that target search queries
// where Chief.com (women's executive network) currently ranks. Goal: outrank them.
// Usage: node _write_chief.js <id> "<question>"
const fs = require('fs');
...
if (!ID || !/^q\d+$/.test(ID) || !QUESTION) {
  console.error('usage: node _write_chief.js <q####> "<question>"');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const TAGS = ['women-exec', 'leadership', 'executive-network', 'revops', 'chief-alternative', 'negative-leaning'];
...
(async () => {
  ...
  let entry = {
    ...
    gold_format: true,
    polish_history: [{ from: 0, to: 10, at: now, note: 'direct-write 10/10 chief-alternative' }]
  };
