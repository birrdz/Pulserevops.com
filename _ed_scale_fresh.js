...
(async()=>{
  ...
  // Read candidates from the FULL index (library-list caps the returned set, which
  // starved this engine to a ~957-entry window; the full index has ~15.6k fresh).
  const d=(await store.get('_index.json',{type:'json'}))||{entries:[]};
  let cand=(d.entries||[]).filter(e=>e&&e.question&&e.has_answer!==false&&!e.noindex&&!e.copied_from&&e.source!=='editorial'&&!e.editorial_style&&/^[a-z]+\d+$/.test(e.id)&&!/^tl9\d/.test(e.id)&&!/^vq_/.test(e.id));
  // FRESH-ONLY: never-editorialized entries, newest first — advances coverage (no re-churn).
  cand.sort((a,b)=>((b.ts||0)-(a.ts||0)));
  // FRESH-ONLY: walk the whole candidate pool (newest first); the per-blob
  // editorial_style/answer_orig check below skips already-editorialized entries
  // WITHOUT counting them, so we keep pulling until COUNT *new* ones are written.
  const picks=cand;
  console.log('faithful story rewrite: target '+COUNT+' FRESH of '+picks.length+' candidates, par '+PAR);
  let i=0,done=0,fail=0,skip=0;
  async function worker(){while(i<picks.length){ if(done>=COUNT) return; const k=i++;const s=picks[k];const rec=RECIPES[k%RECIPES.length];
    try{
      const blob=await store.get('answers/'+s.id+'.json',{type:'json'});if(!blob){fail++;continue;}
      // Already editorialized? skip it (don't re-churn, don't count toward COUNT).
      if(blob.editorial_style||blob.answer_orig){skip++;continue;}
      const source=stripFooter(blob.answer);
      ...
      done++; if(done%20===0)console.log('  '+done+'/'+COUNT+' fresh (skipped '+skip+' done-already; last '+s.id+' '+body.split(/\s+/).length+'w)');
    }catch(err){fail++;console.log('  ERR '+s.id+' '+(err.message||err));}
  }}
  await Promise.all(Array.from({length:PAR},()=>worker()));
  await fetch('https://pulserevops.com/.netlify/functions/pulse-progress-notify?key=pulsemachine-writer-2026',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subject:`PULSE — ${done} NEW editorials written (facts preserved)`,html:`<h2>${done} fresh entries editorialized (${skip} already-done skipped, ${fail} failed)</h2><p>Same information, storyteller delivery. Same questions, same IDs — SEO preserved. Browse <a href="https://pulserevops.com/thoughts">My Thoughts</a>.</p>`})});
  console.log('DONE fresh-editorials done='+done+' skipped='+skip+' fail='+fail);
