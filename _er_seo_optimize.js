// SEO-optimize all live er#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _er_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const erRows = (idx.entries || []).filter((e) => e && /^er\d+$/i.test(e.id));
  erRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of erRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: ER_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/electronic-reviews/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(ER_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: ER_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: ER_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_er_seo_optimize_report.json', JSON.stringify(report, null, 1));
