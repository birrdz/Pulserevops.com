...
//
// Usage: node _write_sp.js <sp####> "<title>" [topic-slug]
...
if (!ID || !/^sp\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_sp.js <sp####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['speech', 'toast', 'original-speech', 'famous-speech'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/speeches/${ID}`, indexnow: indexed }));
