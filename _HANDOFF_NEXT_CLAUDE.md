...
gm reimaged slot files · gm0030 (blob live) · `vs-expert-verify.js` grader fix · renderer `cro-bar` link · `hire.html`.

### 🎏 gm = DO-NOT-TOUCH after this pass (owner) — `image_run_completed.json` = ["gm"]. This image-fix pass is the owner-ordered exception; re-mark done when finished.

### 💾 GITHUB BACKUP STATE (2026-07-08) — repo is BLOATED, backups split
- Remote: `github.com/birrdz/Pulserevops.com`, branch **`backup-2026-06-26`**. That branch was WEEKS behind → a full commit vs it = **59,002 files / 4.77 GiB** (43,161 `assets/qa` generated images + 12,475-file `_site_deploy` BUILD dir). A single full `git push` over HTTPS **aborts** (`curl 55 Send failure`).
- **What I did**: committed + pushed **code/spec/content ONLY** (~1,708 text files — all today's work: `image-relevance-gate.js`, `_gm_top10_reimage.js`, `vs-expert-verify.js`, `pulse-machine-entry.js`, `hire.html`, spec + handoff). Set `http.postBuffer=1048576000`, `http.version=HTTP/1.1`. Excluded `assets/`, `_site_deploy/`, `_facecard_pool/`, `img/` from this push (reproducible / build output).
- **TODO (owner/next session)**: (a) **gitignore `_site_deploy/`** — it's build output, should never be in git. (b) Move `assets/qa/*.jpg` to **git LFS** OR gitignore (regenerable by the pipeline) so backups stay fast. (c) The 43k images are NOT on GitHub — the LIVE site (Netlify) has them; a fresh clone will preview with missing local images until re-synced/regenerated. (d) Consider a fresh backup branch dated today.
