// SEO-optimize all live es#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _es_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const esRows = (idx.entries || []).filter((e) => e && /^es\d+$/i.test(e.id));
  esRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of esRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: ES_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/estates/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(ES_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: ES_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: ES_STATIC_URLS,
    sample: results[0],
  };
  fs.writeFileSync('C:/Users/koryj/website/_es_seo_optimize_report.json', JSON.stringify(report, null, 2));
