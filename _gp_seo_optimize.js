// SEO-optimize all live gp#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _gp_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const gpRows = (idx.entries || []).filter((e) => e && /^gp\d+$/i.test(e.id));
  gpRows.sort((a, b) => parseInt(a.id.replace(/^gp0*/i, ''), 10) - parseInt(b.id.replace(/^gp0*/i, ''), 10));
  ...
  for (const row of gpRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: GP_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/go-to-market-playbooks/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(GP_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: GP_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: GP_STATIC_URLS,
    sample: results.find((r) => /^gp0001$/i.test(r.id)) || results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_gp_seo_optimize_report.json', JSON.stringify(report, null, 1));
