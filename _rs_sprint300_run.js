// Generate + publish rs0051–rs0350 (300 resorts Top-10 entries).
// Usage: node _rs_sprint300_run.js [startId] [--dry-run]
// Image pipeline: DDG-slow (sequential via _write_rs.js) — expect ~2–5 min/entry for Top-10 covers + product cards.
const fs = require('fs');
const { execSync } = require('child_process');
const { buildBody } = require('./_rs_sprint300_bodies');
const { createBatchProgressReporter } = require('./_progress_email');

const QUEUE = JSON.parse(fs.readFileSync('C:/Users/koryj/website/_rs_sprint300.json', 'utf8'));
const PROG = 'C:/Users/koryj/website/_rs_sprint300_progress.json';
const LOG = 'C:/Users/koryj/website/_rs_sprint300_run.log';
const DRY = process.argv.includes('--dry-run');
const startArg = process.argv.find((a) => /^rs\d+$/i.test(a));
...
function publish(id, title, slug, body) {
  ...
  const out = execSync(`node _write_rs.js ${id} "${titleEsc}" ${slug}`, {
...
(async () => {
  const progress = createBatchProgressReporter({
    label: 'PULSE rs sprint300',
    total: items.length,
    interval: 10,
    pillarUrl: 'https://pulserevops.com/resorts',
  });
  await progress.start(`<p>Publishing <b>${items.length}</b> resort entries (${items[0]?.id || 'none'} → ${items[items.length - 1]?.id || 'none'})</p>`);
  ...
  await progress.complete(`<p>Published <b>${pub}</b>, rejected <b>${rej}</b>. Run <code>node _rs_finish.js</code> for SEO hub sync + verify + deploy.</p>`);
