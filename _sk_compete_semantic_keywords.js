// Pulse Skill Drills — semantic search + FAQ compete keywords for sk#### entries.
const SK_KEYWORD_PHRASES = require('./_sk_keyword_phrases.json');

const SK_HUB_URL = 'https://pulserevops.com/skills';
const SK_HUB_PAGE = 'skills.html';
const SK_STATIC_URLS = [SK_HUB_URL];
...
function phraseToFaq(phrase) {
  ...
  const a = `Pulse Skill Drills ranks ${phrase} with Best Overall and Best Value picks and a buyer decision tree for 2027.`;
  return { q, a };
}

const SEMANTIC_FAQ_HUB_SCHEMA = SK_KEYWORD_PHRASES.slice(0, 48).map(phraseToFaq);

const COMPETE_DISPLAY_KEYWORDS = [
  'Semantic Search: Pulse Skill Drills FAQ',
  'Pulse Skill Drills FAQ structured data',
  'Pulse Skill Drills semantic search optimization',
  'Pulse Skill Drills Top-10 Rankings',
  'Best Overall vs Best Value',
  'top-10 rankings 2027',
  ...SK_KEYWORD_PHRASES,
];

const GLOBAL_TAGS = ["pulse-skills","skill-drills","sales-drills","top-10","best-of-2027"];

function keywordsFromTitle(title) {
  const t = String(title || '').toLowerCase();
  const hits = [];
  for (const phrase of SK_KEYWORD_PHRASES) {
  ...
  if (!hits.length) hits.push(...SK_KEYWORD_PHRASES.slice(0, 8).map(slugify));
  return hits;
}
...
function tagsForEntry(id, question, existingTags = []) {
  ...
  const allSlugs = SK_KEYWORD_PHRASES.map(slugify);
...
function hubKeywordItemListJsonLd(hubUrl) {
  return {
    ...
    name: 'Pulse Skill Drills Search Index',
    numberOfItems: SK_KEYWORD_PHRASES.length,
    itemListElement: SK_KEYWORD_PHRASES.map((phrase, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: phrase,
...
function entrySeoBrandKeywords(question) {
  return Array.from(new Set([
    ...
    'Pulse Skill Drills review',
    'Pulse Skill Drills reviews',
    'Pulse Skill Drills rankings',
  ]));
}
