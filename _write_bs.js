// Direct-write for the Sales Book Summaries (Cliff Notes) pillar — bs####.
// Modeled on _write_tk.js. Structurally a qa-pillar entry (gold template).
//
// Usage: node _write_bs.js <bs####> "<title>" <author-slug>
// Reads body from C:/Users/koryj/<id>_answer.md
//
// GOLD-FORMAT GATE: refuses to publish if the body scores < 10/12 on shared
// grader. Per-pillar override: bs#### requires 1500-word floor (not 1100) —
// enforced in grade-entry.js via pillarOf('bs####') → 'booksummary'.
const fs = require('fs');
...
const AUTHOR_SLUG = (process.argv[4] || '').toLowerCase();
if (!ID || !/^bs\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_bs.js <bs####> "<title>" <author-slug>');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['book-summary', 'cliff-notes', 'sales-book', 'sales-book-summary', 'sales-leadership'];
if (AUTHOR_SLUG) BASE_TAGS.push(AUTHOR_SLUG);
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade book-summary' : 'direct-write 10/10 book-summary' }
    ]
  };
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/sales-book-summaries/${ID}`, indexnow: indexed }));
