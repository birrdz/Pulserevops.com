// Pulse Football Recruiting — semantic search + FAQ compete keywords for hf#### entries.
const HF_KEYWORD_PHRASES = require('./_hf_keyword_phrases.json');
...
function phraseToFaq(phrase) {
  ...
  const a = `Pulse Football Recruiting ranks ${phrase} with Best Overall and Best Value picks and a buyer decision tree for 2027.`;
  return { q, a };
}

const SEMANTIC_FAQ_HUB_SCHEMA = HF_KEYWORD_PHRASES.slice(0, 48).map(phraseToFaq);

const COMPETE_DISPLAY_KEYWORDS = [
  'Semantic Search: Pulse Football Recruiting FAQ',
  'Pulse Football Recruiting FAQ structured data',
  'Pulse Football Recruiting semantic search optimization',
  'Pulse Football Recruiting Top-10 Rankings',
  'Best Overall vs Best Value',
  'top-10 rankings 2027',
  ...HF_KEYWORD_PHRASES,
];

const GLOBAL_TAGS = ["pulse-football-recruiting","hs-football-recruiting","football-recruiting","top-10","best-of-2027"];

function keywordsFromTitle(title) {
  const t = String(title || '').toLowerCase();
  const hits = [];
  for (const phrase of HF_KEYWORD_PHRASES) {
  ...
  if (!hits.length) hits.push(...HF_KEYWORD_PHRASES.slice(0, 8).map(slugify));
  return hits;
}
...
function tagsForEntry(id, question, existingTags = []) {
  ...
  const allSlugs = HF_KEYWORD_PHRASES.map(slugify);
...
function hubKeywordItemListJsonLd(hubUrl) {
  return {
    ...
    name: 'Pulse Football Recruiting Search Index',
    numberOfItems: HF_KEYWORD_PHRASES.length,
    itemListElement: HF_KEYWORD_PHRASES.map((phrase, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: phrase,
...
function entrySeoBrandKeywords(question) {
  return Array.from(new Set([
    ...
    'Pulse Football Recruiting review',
    'Pulse Football Recruiting reviews',
    'Pulse Football Recruiting rankings',
  ]));
}
