// SEO-optimize all live dn#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _dn_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const dnRows = (idx.entries || []).filter((e) => e && /^dn\d+$/i.test(e.id));
  dnRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of dnRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: DN_KEYWORD_PHRASES.length,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(DN_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: DN_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: DN_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_dn_seo_optimize_report.json', JSON.stringify(report, null, 1));
