// Sync Pulse Movies hub meta + JSON-LD. Usage: node _mv_seo_sync_hub.js
const fs = require('fs');
const path = require('path');
const { MV_HUB_PAGE, MV_HUB_URL, hubMetaKeywords, hubFaqJsonLd, hubKeywordItemListJsonLd, MV_KEYWORD_PHRASES } = require('./_mv_compete_semantic_keywords');
const { pingIndexNowUrlList } = require('./netlify/functions/lib/indexnow-ping-entry');

const HUB = path.join(__dirname, MV_HUB_PAGE);
...
const collectionLd = {
  '@context': 'https://schema.org', '@type': 'CollectionPage', '@id': `${MV_HUB_URL}#webpage`,
  name: 'Pulse Movies', url: MV_HUB_URL, description: "Top-10 movie rankings by genre, decade, and mood with Best Overall + Best Value picks.",
  keywords: phrases.slice(0, 24).join(', '),
};
const ldBlock = `<script type="application/ld+json">\n${JSON.stringify(collectionLd, null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubFaqJsonLd(), null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubKeywordItemListJsonLd(MV_HUB_URL), null, 2)}\n</script>`;
...
console.log('hub synced', MV_HUB_PAGE, 'phrases', MV_KEYWORD_PHRASES.length);
(async () => { const ping = await pingIndexNowUrlList([MV_HUB_URL]); console.log('hub IndexNow', JSON.stringify(ping)); })().catch(console.error);
