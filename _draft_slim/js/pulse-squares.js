...
(function (global) {
  ...
  function cardHtml(e, opts) {
    ...
    return '<a class="rcard" href="' + esc(href) + '" style="--tc:' + col + '" title="' + tip + '">'
      + '<span class="rcard-imgwrap"><img loading="lazy" draggable="false" src="' + img + '" alt="' + tip + '" data-fb="' + fallback + '" onerror="window.PulseSquares&&PulseSquares.imgFail(this)"></span>'
  ...
  function wireGrabScroll(sc, opts) {
    ...
    sc.addEventListener('mouseenter', function () { pause(null); });
    sc.addEventListener('mouseleave', function () { if (!drag) pause(1200); });

    if (auto) {
      setInterval(function () {
        if (paused || drag || document.hidden) return;
        if (Date.now() - lastInteract < idleMs) return;
        var max = Math.max(0, sc.scrollWidth - sc.clientWidth);
        if (max <= 0) return;
        if (sc.scrollLeft >= max - 1) {
          sc.scrollLeft = 0; // soft restart — no cloned cards / no on-screen image dupes
          return;
        }
        sc.scrollLeft += 0.85;
      }, 16);
  ...
  function paintRow(el, entries, opts) {
    ...
    wireGrabScroll(el.querySelector('.recentscroll'), opts);

    // Every 30 min — randomize square order so people don't always see the same stuff.
    if (opts.reshuffle !== false && !el.__psqReshuffleTimer) {
      el.__psqReshuffleTimer = setInterval(function () {
        if (document.hidden) return;
        var src = el.__psqSource || [];
        var o = el.__psqOpts || opts;
        paintRow(el, src, Object.assign({}, o, { usedImages: null }));
      }, opts.reshuffleMs || 30 * 60 * 1000);
    }
  }
