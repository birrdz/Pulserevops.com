// SEO-optimize all live fr#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _fr_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const frRows = (idx.entries || []).filter((e) => e && /^fr\d+$/i.test(e.id));
  frRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of frRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: FR_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/franchises/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(FR_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: FR_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: FR_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_fr_seo_optimize_report.json', JSON.stringify(report, null, 2));
