// Aquarium dual-gold redo progress email — every 2 min (owner 2026-07-06).
const fs = require('fs');
...
const INTERVAL_MS = parseInt(process.env.AQ_PROGRESS_EMAIL_MS || String(2 * 60 * 1000), 10);
const PROGRESS_F = WD + '/_aq_gold_redo_progress.json';
const STATE_F = WD + '/_aq_gold_redo_batch_state.json';
const LOG_F = WD + '/_aq_gold_redo_progress_email.log';
const PING_STATE_F = WD + '/_aq_gold_redo_progress_email_ping.json';
const STOP = WD + '/_aq_gold_redo_progress_email_stop.flag';
...
const { pillarUrl } = (() => {
  ...
  const hubMap = { aq: 'aquariums', sw: 'software', tk: 'software', er: 'electronicreview', pt: 'pets', cr: 'crabbing', fs: 'fishing', ce: 'knowledge' };
  function pillarUrl(id) {
    const hub = hubMap[prefixOf(id)] || 'knowledge';
    return hub === 'knowledge' ? 'https://pulserevops.com/knowledge/' + id : 'https://pulserevops.com/' + hub + '/' + id;
  }
  return { pillarUrl };
})();
...
async function send() {
  ...
  const subject = complete
    ? '🐠 Aquarium pillar COMPLETE — paused · ' + rkDoneIds.length + ' Top 10 + ' + qaDoneIds.length + ' Q&A'
    : firstSend
      ? '🐠 Aquarium dual-gold redo — progress emails every 2 min'
      : '🐠 Aquarium · ' + rkDoneIds.length + ' Top 10 + ' + qaDoneIds.length + ' Q&A · ' + remaining + ' left' + (p.currentId ? ' · ' + p.currentId : '');

  const html = `<div style="font-family:system-ui,Arial,sans-serif;font-size:15px;line-height:1.6;color:#15110d">
    <p style="font-size:18px;font-weight:800;margin:0 0 8px">${complete ? '✅ Aquarium pillar COMPLETE — paused' : firstSend ? 'Aquarium dual-gold redo running' : '🐠 Aquarium progress'}</p>
    ...
    <p style="margin:0 0 12px;color:#6b5d49">🔒 <b>Golden template law:</b> Top 10 → <a href="https://pulserevops.com/aquariums/aq1158">aq1158</a> · Q&amp;A → <a href="https://pulserevops.com/knowledge/q11133">q11133</a> · progress ping every <b>2 min</b> with every finished link</p>
    ...
    <p style="color:#8a7a63;font-size:12px;margin-top:12px">${new Date().toLocaleString()} · next ping in 2 min · stop: _aq_gold_redo_progress_email_stop.flag</p>
  </div>`;
...
(async () => {
  ...
  log(ONCE ? 'one-shot' : 'every 2 min to ' + RECIPIENT);
  if (ONCE) {
