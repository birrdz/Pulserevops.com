// Sync Pulse Style hub meta + JSON-LD. Usage: node _sy_seo_sync_hub.js
const fs = require('fs');
const path = require('path');
const { SY_HUB_PAGE, SY_HUB_URL, hubMetaKeywords, hubFaqJsonLd, hubKeywordItemListJsonLd, SY_KEYWORD_PHRASES } = require('./_sy_compete_semantic_keywords');
const { pingIndexNowUrlList } = require('./netlify/functions/lib/indexnow-ping-entry');

const HUB = path.join(__dirname, SY_HUB_PAGE);
...
const collectionLd = {
  '@context': 'https://schema.org', '@type': 'CollectionPage', '@id': `${SY_HUB_URL}#webpage`,
  name: 'Pulse Style', url: SY_HUB_URL, description: "Top-10 fashion and grooming rankings with Best Overall + Best Value picks.",
  keywords: phrases.slice(0, 24).join(', '),
};
const ldBlock = `<script type="application/ld+json">\n${JSON.stringify(collectionLd, null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubFaqJsonLd(), null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubKeywordItemListJsonLd(SY_HUB_URL), null, 2)}\n</script>`;
...
console.log('hub synced', SY_HUB_PAGE, 'phrases', SY_KEYWORD_PHRASES.length);
(async () => { const ping = await pingIndexNowUrlList([SY_HUB_URL]); console.log('hub IndexNow', JSON.stringify(ping)); })().catch(console.error);
