...
async function load(id){FACE=null;INT=[];DR=[];E=(await j('/api/next'+(id?('?id='+encodeURIComponent(id)):'')));var s=E.stats;document.getElementById('sub').innerHTML='📚 <b style="color:#4ce07a">'+s.done.toLocaleString()+'</b> done · <b style="color:#e8c874">'+s.remaining.toLocaleString()+'</b> to go · '+s.total.toLocaleString()+' Q&amp;A';
  ...
  document.getElementById('idlab').innerHTML='<b>'+E.advId+'</b> · ref '+E.id+' · "'+escH((E.q||'').slice(0,60))+'"';
...
async function fixOne(){
  var d=await j('/api/next');if(!d||!d.id)return{done:true};var id=d.id;log('▶ '+id+' ('+d.advId+') gate '+d.score+'/13');
  // 1. content
  if(!d.pass){stage('🔨 '+id+' — content to 13/13…');var r=await j('/api/build?id='+encodeURIComponent(id));if(r&&r.ok&&r.pass){await j('/api/pubcontent',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:id,body:r.body})});log('✅ '+id+' content 13/13 · '+r.words+'w')}else{log('⏭ '+id+' content stuck '+((r&&r.after)||'?')+'/13 — skipping entry');await j('/api/skip',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:id})});return{id:id,skipped:'content'}}}
