// Immediate email when a CRO Pulse Tools (tl) entry passes 13/13 + gold audit.
const fs = require('fs');
const WD = 'C:/Users/koryj/website';
const RECIPIENT = 'koryjordanwhite@gmail.com';
const KEY_CACHE = WD + '/_ask_owner_key.cache';
const LOG_F = WD + '/_tl_1313_entry_email.log';
...
async function notifyTl1313Entry(opts) {
  opts = opts || {};
  const id = String(opts.id || '').toLowerCase();
  if (!/^tl\d+$/i.test(id)) return { sent: false, reason: 'not_tl' };
  ...
  const dupNote = dupFrom ? ' · dup from ' + dupFrom : '';
  const subject = (forceTest ? '[TEST] ' : '') + '✅ 13/13 · ' + id + ' · ' + tplLabel + dupNote;
  ...
  const html = `<div style="font-family:system-ui,Arial,sans-serif;font-size:15px;line-height:1.6;color:#15110d;max-width:640px">
    ...
    <p style="margin:0 0 6px;font-size:13px;color:#0b57d0;font-weight:700">${isQa ? 'CRO Pulse Tools Q&amp;A · q11133 gold law' : 'Top 10 ranking · aq1158 gold law'}</p>
    ...
    <table style="border-collapse:collapse;font-size:14px">
      ...
      ${dupFrom ? '<tr><td style="padding:2px 12px 2px 0;color:#8a7a63">Dup</td><td>' + esc(dupFrom) + '</td></tr>' : ''}
    </table>
    <p style="color:#8a7a63;font-size:12px;margin-top:14px">${new Date().toLocaleString()} · CRO Pulse Tools gold redo</p>
  </div>`;
  ...
  log('sent · ' + id + ' · ' + tplLabel + dupNote + ' · ' + wc + 'w');
  return { sent: true, subject };
}

module.exports = { notifyTl1313Entry, RECIPIENT, templateLabel };
...
if (require.main === module) {
  ...
  if (!testQa) {
    console.log('Usage: node _tl_1313_entry_email.js --test-qa [--id=tl1234] [--force]');
    process.exit(1);
  }
  (async () => {
    ...
    const id = String(testId || 'tl9001').toLowerCase();
    ...
    const r = await notifyTl1313Entry({
      id,
      title,
      body: e.answer,
      template: 'qa',
