// SEO-optimize all live ga#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _ga_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const gaRows = (idx.entries || []).filter((e) => e && /^ga\d+$/i.test(e.id));
  gaRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of gaRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: GA_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/gatherings/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(GA_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: GA_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: GA_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_ga_seo_optimize_report.json', JSON.stringify(report, null, 1));
