// Pulse Style — semantic search + FAQ compete keywords for sy#### entries.
const SY_KEYWORD_PHRASES = require('./_sy_keyword_phrases.json');

const SY_HUB_URL = 'https://pulserevops.com/style';
const SY_HUB_PAGE = 'style.html';
const SY_STATIC_URLS = [SY_HUB_URL];
...
function phraseToFaq(phrase) {
  ...
  const a = `Pulse Style ranks ${phrase} with Best Overall and Best Value picks and a buyer decision tree for 2027.`;
  return { q, a };
}

const SEMANTIC_FAQ_HUB_SCHEMA = SY_KEYWORD_PHRASES.slice(0, 48).map(phraseToFaq);

const COMPETE_DISPLAY_KEYWORDS = [
  'Semantic Search: Pulse Style FAQ',
  'Pulse Style FAQ structured data',
  'Pulse Style semantic search optimization',
  'Pulse Style Top-10 Rankings',
  'Best Overall vs Best Value',
  'top-10 rankings 2027',
  ...SY_KEYWORD_PHRASES,
];

const GLOBAL_TAGS = ["pulse-style","style","fashion","grooming","top-10","best-of-2027"];

function keywordsFromTitle(title) {
  const t = String(title || '').toLowerCase();
  const hits = [];
  for (const phrase of SY_KEYWORD_PHRASES) {
  ...
  if (!hits.length) hits.push(...SY_KEYWORD_PHRASES.slice(0, 8).map(slugify));
  return hits;
}
...
function tagsForEntry(id, question, existingTags = []) {
  ...
  const allSlugs = SY_KEYWORD_PHRASES.map(slugify);
...
function hubKeywordItemListJsonLd(hubUrl) {
  return {
    ...
    name: 'Pulse Style Search Index',
    numberOfItems: SY_KEYWORD_PHRASES.length,
    itemListElement: SY_KEYWORD_PHRASES.map((phrase, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: phrase,
