// Direct-write for the Tech Stacks pillar (per-industry recommended RevOps/sales
// software stacks). Modeled on _write_ik.js (Industry KPIs) since "recommended
// stack per industry" is structurally the same as "key KPIs per industry".
//
// Usage: node _write_tk.js <tk####> "<question>" <industry-slug>
// Reads body from C:/Users/koryj/<id>_answer.md
//
// GOLD-FORMAT GATE: refuses to publish if the body scores < 10/12 on the shared
// grader. tk IDs grade under the generic (qa-style) structural checks: Direct
// Answer header, 6+ H2, 2 mermaids, FAQ, 5+ sources, 3+ named companies, 8+ bold
// spans, no banned phrases, >=1100 words. Use --force to override (rarely needed).
const fs = require('fs');
...
if (!ID || !/^tk\d+$/.test(ID) || !QUESTION) {
  console.error('usage: node _write_tk.js <tk####> "<question>" <industry-slug>');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['tech-stack', 'revops-tools', 'sales-stack', 'software-recommendations', 'revenue-operations'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade tech-stack' : 'direct-write 10/10 tech-stack' }
    ]
  };
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/tech-stacks/${ID}`, indexnow: indexed }));
