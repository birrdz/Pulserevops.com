// SEO-optimize all live bo#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _bo_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const boRows = (idx.entries || []).filter((e) => e && /^bo\d+$/i.test(e.id));
  boRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of boRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: BO_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/buildouts/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(BO_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: BO_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: BO_STATIC_URLS,
    sample: results[0],
  };
  fs.writeFileSync('C:/Users/koryj/website/_bo_seo_optimize_report.json', JSON.stringify(report, null, 2));
