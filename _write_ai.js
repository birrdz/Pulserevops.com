// Direct-write for the AI Infrastructure pillar — ai####.
// Modeled on _write_co.js. Top-10 "best AI tools for <task>" rankings
// ("Top 10 AI Tools for Animation in 2027", "Top 10 AI Tools for Creating
// Resumes in 2027", etc.) with Best Overall + Best Value highlights inside
// each list. Reuses the Electronic-Reviews Top-10 skeleton + grader ruleset.
//
// Usage: node _write_ai.js <ai####> "<title>" [topic-slug]
// Reads body from C:/Users/koryj/<id>_answer.md
//
// GOLD-FORMAT GATE: refuses to publish if the body scores < 10/12 on shared
// grader. ai#### maps to the electronicreview ruleset: 1800-word floor, 10+
...
if (!ID || !/^ai\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_ai.js <ai####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['ai-tool-review', 'top-10', 'ai-tools', 'ai-infrastructure', 'best-of-2027'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade ai-tools' : 'direct-write 10/10 ai-tools' }
    ]
  };
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/ai-infrastructure/${ID}`, indexnow: indexed }));
