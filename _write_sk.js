...
//
// Usage: node _write_sk.js <sk####> "<title>" [topic-slug]
...
if (!ID || !/^sk\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_sk.js <sk####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['skill', 'skill-drill', 'workshop', 'training'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/skills/${ID}`, indexnow: indexed }));
