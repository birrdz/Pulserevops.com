...
(async()=>{
  ...
  const allCa=(idx.entries||[]).map(e=>e.id).filter(id=>/^rs\d+$/.test(id)).sort((a,b)=>num(a)-num(b));
  const weak=[];
  ...
  async function worker(){
    while(cursor<weak.length){
      ...
      if(Date.now()-lastEmail>15*60*1000){lastEmail=Date.now();await emailOwner(`PULSE rs DDG backfill: ${fixed}/${weak.length}`,`<p>Resorts image backfill (keyless/DuckDuckGo) in progress: <b>${fixed}</b>/${weak.length} entries updated, ${improved} improved, ${addedImgs} images added.</p>`);}
    }
  }
  await Promise.all(Array.from({length:CONCURRENCY},worker));
  console.log(`\nDDG DONE. updated=${fixed} improved=${improved} imagesAdded=${addedImgs}`);
  fs.writeFileSync('C:/Users/koryj/website/_rs_ddg_result.json',JSON.stringify({targets:weak.length,fixed,improved,addedImgs},null,1));
  await emailOwner(`PULSE rs DDG backfill COMPLETE`,`<p>Resorts keyless image backfill finished. ${improved} entries improved, ${addedImgs} real images added across ${weak.length} targeted entries.</p><p>https://pulserevops.com/resorts</p>`);
