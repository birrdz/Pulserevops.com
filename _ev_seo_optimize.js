// SEO-optimize all live ev#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _ev_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const evRows = (idx.entries || []).filter((e) => e && /^ev\d+$/i.test(e.id));
  evRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of evRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: EV_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/events/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(EV_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: EV_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: EV_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_ev_seo_optimize_report.json', JSON.stringify(report, null, 1));
