...
//
// Usage: node _write_tl.js <tl####> "<title>" [topic-slug]
// Reads body from C:/Users/koryj/<id>_answer.md
//
// GOLD-FORMAT GATE: refuses to publish if the body scores < 10/12 on shared
// grader. Publish pipeline auto-ensures Image LAW (cover + 10 product imgs)
// via _write_lib.prepareBodyForGrade before grading.
const fs = require('fs');
...
if (!ID || !/^tl\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_tl.js <tl####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['tools', 'top-10', 'sales-tools', 'calculators-2027'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade tool' : 'direct-write 10/10 tool' }
    ]
  };
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/tools/${ID}`, indexnow: indexed }));
