// Direct-write for the HS Football Recruiting pillar — hf####.
// Top-10 recruiting ranking guides, HEAVY on "who to use to get recruited"
// (recruiting services, camps, combines, trainers, evaluators) and "how to get
// recruited" (steps, film, exposure, contacting coaches). Reuses the
// Electronic-Reviews Top-10 skeleton + grader ruleset: 1800-word floor, 10
// numbered picks, 🏆 BEST OVERALL + 💎 BEST VALUE pills, 1 mermaid, FAQ, Sources.
//
// Usage: node _write_hf.js <hf####> "<title>" [topic-slug]
...
if (!ID || !/^hf\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_hf.js <hf####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['football-recruiting', 'top-10', 'recruiting', 'high-school-football', 'best-of-2027'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade recruiting' : 'direct-write 10/10 recruiting' }
    ]
  };
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/highschool-football-recruiting/${ID}`, indexnow: indexed }));
