// Direct-write for the Resorts pillar — rs####.
// Top-10 resort rankings with Best Overall + Best Value highlights.
// Usage: node _write_rs.js <rs####> "<title>" [topic-slug]
...
if (!ID || !/^rs\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_rs.js <rs####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['resorts', 'top-10', 'best-of-2027', 'luxury-resort'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade resorts' : 'direct-write 10/10 resorts' },
    ],
  };
