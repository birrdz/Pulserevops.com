...
async function loadLib(){el('ordbtn').textContent=libOrder==='old'?'oldest first':'newest first';var pil=el('libpillar')?el('libpillar').value:'';var d=await fetch('/library?order='+libOrder+'&pillar='+encodeURIComponent(pil)).then(function(r){return r.json()});LIB=d.items||[];el('libcount').textContent=(pil?(PILLAR_NAME[pil]||pil.toUpperCase())+': ':'')+'('+((d.pillarTotal!=null?d.pillarTotal:d.total)||LIB.length).toLocaleString()+' entries)';LIBSHOW=300;renderLib()}
function renderLib(){var q=(el('libq').value||'').toLowerCase();var hd=el('hidedone')&&el('hidedone').checked;var rows=LIB.filter(function(e){if(hd&&e.face)return false;return !q||e.id.toLowerCase().indexOf(q)>=0||(e.q||'').toLowerCase().indexOf(q)>=0}).slice(0,LIBSHOW);function dot(on){return '<span style="color:'+(on?'#4ce07a':'#40454d')+';font-weight:900">●</span>'}el('liblist').innerHTML=rows.map(function(e){return '<div class=lrow onclick="openEntry(\\''+e.id+'\\')" title="'+e.id+'">'+dot(e.face)+dot(e.content)+dot(e.internal)+' <span style="color:#a9d8ff">'+e.advId+'</span> <span style="color:#6b7280;font-size:11px">'+e.id+'</span> '+(e.q||'(blank)').replace(/</g,'&lt;')+'</div>'}).join('')||'<div class=lrow>no matches</div>';}
function layout(){var h=el('top').offsetHeight;el('frameWrap').style.top=h+'px'}
window.addEventListener('resize',layout);
ping();setInterval(ping,5000);populatePillars();setTimeout(layout,150);setInterval(layout,2500);
</script>`;
