// SEO-optimize all tl#### entries. Usage: node _tl_seo_optimize.js [--dry-run]
const fs = require('fs');
const { getStore } = require('@netlify/blobs');
const { tagsForEntry, entrySeoBrandKeywords, semanticFaqForTitle, TL_STATIC_URLS, TL_KEYWORD_PHRASES } = require('./_tl_compete_semantic_keywords');
...
(async () => {
  ...
  for (const row of rows) {
    ...
    if (!DRY && did) {
      const now = Date.now();
      await store.setJSON('answers/' + row.id + '.json', { ...entry, tags: newTags, seo_brand_keywords: newDisplay, seo_semantic_faq_questions: newFaq, seo_semantic_faq: true, seo_keyword_phrase_count: TL_KEYWORD_PHRASES.length, seo_optimized_at: now });
  ...
  if (!DRY) await pingIndexNowUrlList(TL_STATIC_URLS);
  const report = { ok: true, prefix: 'tl', count: rows.length, changed, phrases: TL_KEYWORD_PHRASES.length };
  fs.writeFileSync('C:/Users/koryj/website/_tl_seo_optimize_report.json', JSON.stringify(report, null, 1));
