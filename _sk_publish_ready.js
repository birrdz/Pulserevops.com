...
const Q=JSON.parse(fs.readFileSync('C:/Users/koryj/_sk_sprint_queue.json','utf8'));
let pub=0,rej=0,skip=0;
for(const e of Q){const md='C:/Users/koryj/'+e.id+'_answer.md';if(!fs.existsSync(md)){skip++;continue;}
try{const out=execFileSync('node',['_write_sk.js',e.id,e.title,e.slug],{encoding:'utf8',stdio:['ignore','pipe','pipe']});const line=out.trim().split('\n').pop();
