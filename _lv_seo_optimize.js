// SEO-optimize all lv#### entries. Usage: node _lv_seo_optimize.js [--dry-run]
const fs = require('fs');
const { getStore } = require('@netlify/blobs');
const { tagsForEntry, entrySeoBrandKeywords, semanticFaqForTitle, LV_STATIC_URLS, LV_KEYWORD_PHRASES } = require('./_lv_compete_semantic_keywords');
...
(async () => {
  ...
  for (const row of rows) {
    ...
    if (!DRY && did) {
      const now = Date.now();
      await store.setJSON('answers/' + row.id + '.json', { ...entry, tags: newTags, seo_brand_keywords: newDisplay, seo_semantic_faq_questions: newFaq, seo_semantic_faq: true, seo_keyword_phrase_count: LV_KEYWORD_PHRASES.length, seo_optimized_at: now });
  ...
  if (!DRY) await pingIndexNowUrlList(LV_STATIC_URLS);
  const report = { ok: true, prefix: 'lv', count: rows.length, changed, phrases: LV_KEYWORD_PHRASES.length };
  fs.writeFileSync('C:/Users/koryj/website/_lv_seo_optimize_report.json', JSON.stringify(report, null, 1));
