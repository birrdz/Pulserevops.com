// Direct-write for the Collectibles pillar — co####.
// Modeled on _write_ca.js. Top-10 collectible-category rankings ("Top 10
// Vintage Baseball Cards to Collect in 2027", "Top 10 Investment-Grade Watches
...
if (!ID || !/^co\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_co.js <co####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['collectible-review', 'top-10', 'collectible', 'collecting', 'best-of-2027'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade collectible' : 'direct-write 10/10 collectible' }
    ]
  };
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/collectibles/${ID}`, indexnow: indexed }));
