// Direct-write for the towns pillar — tn####.
...
//
// Usage: node _write_tn.js <tn####> "<title>" [topic-slug]
...
if (!ID || !/^tn\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_tn.js <tn####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['town', 'top-10', 'best-of-2027', 'towns'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/towns/${ID}`, indexnow: indexed }));
