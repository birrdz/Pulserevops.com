// Worker that processes one chunk file produced by the splitter.
// Usage: node _tk_analyst_worker.js <chunkIndex>
const fs = require('fs');
...
const CHUNK = parseInt(process.argv[2], 10);
if (isNaN(CHUNK)) { console.error('chunk index required'); process.exit(1); }

const targets = JSON.parse(fs.readFileSync(path.join(__dirname, `_tk_chunk_${CHUNK}.json`), 'utf8'));

const ANALYST_RE = /(Gartner|Forrester|IDC MarketScape|G2 Grid|Pavilion|Bridge Group|ScaleVP|Scale Venture|McKinsey|Bain & Company|\bBCG\b|Deloitte|Verdantix|Bersin|CIMdata|Wood Mackenzie|ICMA|NENA|ACAMS|Aragon Research|Tambellini|Celent|Aite-Novarica|Aite Novarica|\bIIA\b|ISACA|NACM|IATA|FIATA|BIMCO|Drewry|Cantos|Bluefield|Magic Quadrant|Forrester Wave|MarketScape|JBKnowledge|KLAS Research|Bessemer Venture|JLL|HolonIQ|Service Performance|M\+R Benchmarks|Salesforce\.org)/i;
...
function injectMarketContext(body, question) {
  ...
  if (coreStackRe.test(body)) return { body: body.replace(coreStackRe, `$1${block}`), skipped: false, at: 'core-stack' };
  const whyRe = /^(##\s+Why\s+[^\n]*?(?:Works|Operates|Behaves|Runs?|Sells?|Differs?|Different)[^\n]*\n)/im;
  if (whyRe.test(body)) return { body: body.replace(whyRe, `$1${block}`), skipped: false, at: 'why-differently' };
...
async function patchOne(store, id, question) {
  ...
  if (ANALYST_RE.test(ent.answer)) return { id, ok: true, skipped: true, reason: 'already has analyst' };

  const { body: after, at } = injectMarketContext(ent.answer, question);
  ...
  return { id, ok: true, at, score: g.score, wc: g.word_count };
}
...
(async () => {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    if ((i + 1) % 5 === 0 || i === targets.length - 1) {
      ...
      console.log(`[w${CHUNK} ${i + 1}/${targets.length} ${elapsed}s] ok=${results.ok.length} skip=${results.skipped.length} fail=${results.failed.length} last=${results.ok.slice(-1)[0]?.id || results.failed.slice(-1)[0]?.id || ''}`);
    }
  }

  fs.writeFileSync(path.join(__dirname, `_tk_chunk_${CHUNK}_results.json`), JSON.stringify(results, null, 2));
  console.log(`WORKER ${CHUNK} DONE. ok=${results.ok.length} skipped=${results.skipped.length} failed=${results.failed.length}`);
})().catch(e => { console.error(`WORKER ${CHUNK} FATAL`, e && e.stack); process.exit(1); });
