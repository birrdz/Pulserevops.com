...
const PORT = parseInt(process.env.SIM_PORT || '8903', 10);
const SIM = WD + '/sim';
...
async function executeRun(cmd) {
  ...
  if (!fs.existsSync(REPORT_F)) {
    ...
    const scanCode = await runStage(WD + '/sim_scan.js', [scope]);
  ...
  if (fs.existsSync(WD + '/sim_transform.js')) {
    setStatus({ stage: 'transform', phase: 'transforming' });
    const tCode = await runStage(WD + '/sim_transform.js', [scope]);
    ...
    await runStage(WD + '/sim_scan.js', [scope]);
...
async function runScanOnly(scope) {
  ...
  const code = await runStage(WD + '/sim_scan.js', [scope]);
...
async function runStreamAll() {
  ...
  for (const pil of pillars) {
    ...
    const sc = await runStage(WD + '/sim_scan.js', [pil]);
    if (stopRequested) { setStatus({ stage: 'stopped', phase: 'idle' }); running = false; return; }
    if (sc === 0) {
      const sum = readJSON(SUMMARY_F, {});
      ...
      if (bad > 0) await runStage(WD + '/sim_transform.js', [pil]);
...
<div class=dim>The car wash: ① RUN REPORT scans the scope into three sections — <b>Similarity</b>, <b>Sub-13/13</b>, and <b>Stubs</b> → ② GO works the whole list <b>8 at a time</b>: rewrites near-dups until they're distinct, upgrades sub-13 pages to the checklist, writes stubs from scratch, and applies a title + approved cover to anything missing one. Every page must pass the 13/13 gate to publish. Nothing that fails publishes. FORCE STOP kills anything stuck.</div>
...
<div class=card id=fixcard style="display:none"><div class=lab>🔧 Fixing now — 8 at a time · every page must clear the 13/13 checklist (target sim <b id=tgt>30</b>%)</div><div id=fixlist></div></div>
...
async function poll(){try{const s=await(await fetch('/api/status')).json();
 ...
 if((stage==='scan-done'||stage==='done')&&np>0){fx.style.display='block';fx.innerHTML='✨ Fix '+(p.NEAR_DUP||0).toLocaleString()+' similarity + '+(p.SUB13||0).toLocaleString()+' sub-13 + '+(p.STUB||0).toLocaleString()+' stubs — YES, FIX ALL (8 at a time)';}
...
}).listen(PORT, () => console.log('[sim-machine] control panel live on http://localhost:' + PORT + '  (scan wired; transform runs when sim_transform.js present)'));
