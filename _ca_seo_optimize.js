// SEO-optimize all live ca#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _ca_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const caRows = (idx.entries || []).filter((e) => e && /^ca\d+$/i.test(e.id));
  caRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of caRows) {
    ...
    results.push({
      ...
      url: `https://pulserevops.com/cars/${id}`,
      indexnow: indexnow ? { ok: indexnow.ok, pings: indexnow.pings } : null,
    });
    if (changed) console.log(`${id}\ttags=${newTags.length}\tindexnow=${indexnow?.ok ?? 'skip'}`);
  }

  if (!DRY) await store.setJSON('_index.json', idx);

  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(CA_STATIC_URLS);
  ...
  const report = {
    ...
    hubPing,
    hubUrls: CA_STATIC_URLS,
  };
  fs.writeFileSync('C:/Users/koryj/website/_ca_seo_optimize_report.json', JSON.stringify(report, null, 1));
