// Requery the 8 flagged crop-6 questions with better office/business framing, regrade, rebuild sampler6.
const fs = require('fs');
...
const map = JSON.parse(fs.readFileSync('_cat_candidates6.json', 'utf8'));
let ref = {}; try { ref = JSON.parse(fs.readFileSync('_cat_reference6.json', 'utf8')); } catch (e) {}
const OUT = 'assets/qa/_prev';
const FLAG = { SK: [3, 10, 12, 14], SW: [11, 13, 14, 15] };
const MODS = ['fine art', 'oil painting', 'cinematic moody', 'dramatic atmospheric'];
function subj(t) { return String(t).replace(/[?"]/g, '').replace(/^(the\s+\d+\s+best|top\s+\d+|top\s+ten|the\s+best|skill\s+drill:?)\s*/i, '').replace(/\s+in\s+20\d\d/i, '').replace(/\s+/g, ' ').trim(); }
function q(tag, s, n) { const mod = MODS[n % MODS.length]; return tag === 'SK' ? ('artistic professional sales team office meeting handshake ' + s + ' ' + mod) : ('artistic modern technology data software workspace abstract ' + s + ' ' + mod); }
...
(async () => {
  ...
  for (const tag of ['SK', 'SW']) {
    ...
    for (const num of FLAG[tag]) {
      const key = tag + '-' + num; const e = ents[num - 1]; if (!e) continue;
      const query = q(tag, subj(e.question), num);
  ...
  console.log('DONE — 8 flagged requeried. http://localhost:8891/_cat_sampler6.html');
