// SEO-optimize all live st#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _st_seo_optimize.js [--dry-run]
const fs = require('fs');
...
const TOP10_ONLY = !process.argv.includes('--all');

(async () => {
  const store = getStore({ name: 'pulse-machine-library', siteID: SID, token: TOK });
  const idx = (await store.get('_index.json', { type: 'json' })) || { entries: [] };
  let stRows = (idx.entries || []).filter((e) => e && /^st\d+$/i.test(e.id));
  if (TOP10_ONLY) stRows = stRows.filter((e) => parseInt(e.id.slice(2), 10) >= 499);
  stRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of stRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: ST_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/sales-trainings/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(ST_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: ST_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: ST_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_st_seo_optimize_report.json', JSON.stringify(report, null, 1));
