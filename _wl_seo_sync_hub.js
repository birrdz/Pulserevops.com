// Sync Pulse Wellness hub meta + JSON-LD. Usage: node _wl_seo_sync_hub.js
const fs = require('fs');
const path = require('path');
const { WL_HUB_PAGE, WL_HUB_URL, hubMetaKeywords, hubFaqJsonLd, hubKeywordItemListJsonLd, WL_KEYWORD_PHRASES } = require('./_wl_compete_semantic_keywords');
const { pingIndexNowUrlList } = require('./netlify/functions/lib/indexnow-ping-entry');

const HUB = path.join(__dirname, WL_HUB_PAGE);
...
const collectionLd = {
  '@context': 'https://schema.org', '@type': 'CollectionPage', '@id': `${WL_HUB_URL}#webpage`,
  name: 'Pulse Wellness', url: WL_HUB_URL, description: "Top-10 wellness product and routine rankings with Best Overall + Best Value picks.",
  keywords: phrases.slice(0, 24).join(', '),
};
const ldBlock = `<script type="application/ld+json">\n${JSON.stringify(collectionLd, null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubFaqJsonLd(), null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubKeywordItemListJsonLd(WL_HUB_URL), null, 2)}\n</script>`;
...
console.log('hub synced', WL_HUB_PAGE, 'phrases', WL_KEYWORD_PHRASES.length);
(async () => { const ping = await pingIndexNowUrlList([WL_HUB_URL]); console.log('hub IndexNow', JSON.stringify(ping)); })().catch(console.error);
