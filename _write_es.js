// Direct-write for the estates pillar — es####.
// Top-10 luxury estate / real-estate rankings with Best Overall + Best Value highlights.
// Usage: node _write_es.js <es####> "<title>" [topic-slug]
...
if (!ID || !/^es\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_es.js <es####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['estates', 'top-10', 'best-of-2027', 'luxury-real-estate'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade estates' : 'direct-write 10/10 estates' },
    ],
  };
