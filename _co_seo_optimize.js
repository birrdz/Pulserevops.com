// SEO-optimize all live co#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _co_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const coRows = (idx.entries || []).filter((e) => e && /^co\d+$/i.test(e.id));
  coRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of coRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: CO_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/collectibles/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(CO_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: CO_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: CO_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_co_seo_optimize_report.json', JSON.stringify(report, null, 1));
