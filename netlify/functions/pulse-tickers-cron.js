// ════════════════════════════════════════════════════════════════════════
// pulse-tickers-cron — daily scheduled refresh of the homepage ticker
// snapshot (`_tickers_daily.json`). Same logic as the on-demand path in
// pulse-tickers.js, but pre-warms the cache at 00:05 UTC each day so the
// first visitor doesn't pay the compute cost.
// ════════════════════════════════════════════════════════════════════════
...
exports.handler = async () => {
  ...
  if (!store) return { statusCode: 500, body: JSON.stringify({ ok: false, error: 'store init failed' }) };
  try {
    ...
    const snap = {
      ...
      source: 'pulse-tickers-cron',
    };
    await store.setJSON(KEY, snap);
    console.log('[pulse-tickers-cron] OK day=' + today + ' gold[0]=' + (gold[0] || '').slice(0, 60));
    return { statusCode: 200, body: JSON.stringify({ ok: true, day: today, gold_count: gold.length, pain_count: pain.length }) };
  } catch (err) {
    console.error('[pulse-tickers-cron] FAIL', err && err.stack);
    return { statusCode: 500, body: JSON.stringify({ ok: false, error: String(err && err.message) }) };
  }
};

// Daily at 00:05 UTC — runs once per day, refreshes the ticker snapshot.
