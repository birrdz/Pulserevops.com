...
exports.handler = async (event) => {
  ...
  if (!rawTag) {
    ...
    const m = probe.match(/\/knowledge\/tag\/([a-z0-9-]+)/i);
    if (m) rawTag = m[1].toLowerCase();
  }

  // Validate tag — only allow lowercase alphanum + hyphens (matches our tag normalization)
  if (!rawTag || !/^[a-z0-9-]+$/.test(rawTag) || rawTag.length > 50) {
    return { statusCode: 404, body: 'invalid tag' };
  }
  ...
  // 404 if no entries match — don't render an empty page (would hurt SEO)
  if (!matched.length) {
    return {
      statusCode: 404,
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
      body: `<!doctype html><html><head><title>Tag not found</title><meta name="robots" content="noindex"></head><body style="font-family:system-ui;padding:48px;background:#0a0d12;color:#EDE5D8;text-align:center;"><h1 style="color:#FF8C1A;">Tag "${escHtml(rawTag)}" — not in library yet</h1><p>The Machine hasn't researched anything with this tag yet. <a href="/knowledge.html" style="color:#FF8C1A;">Browse the full library</a> or <a href="/themachine" style="color:#FF8C1A;">ask The Machine directly</a>.</p></body></html>`,
    };
  }
