// Direct-write for the nightlife pillar — nl####.
// Top-10 nightlife rankings with Best Overall + Best Value highlights.
// Usage: node _write_nl.js <nl####> "<title>" [topic-slug]
...
if (!ID || !/^nl\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_nl.js <nl####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['nightlife', 'top-10', 'best-of-2027', 'bars-and-clubs'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade nightlife' : 'direct-write 10/10 nightlife' },
    ],
  };
  ...
  console.log(
    JSON.stringify({
      ...
      url: `https://pulserevops.com/nightlife/${ID}`,
      indexnow: indexed,
    })
  );
})().catch((e) => {
