...
const nls=(idx.entries||[]).filter(e=>e&&/^nl\d+$/.test(e.id)).sort((a,b)=>a.id.localeCompare(b.id));
let ok=true;
for(const e of nls){const rec=await s.get('answers/'+e.id+'.json',{type:'json'});const b=rec.answer;const g=gradeEntry(e.id,b);const iss=[];
...
if(e.id==='nl0006')fs.writeFileSync('C:/Users/koryj/_nl0006_view.md',b);}
