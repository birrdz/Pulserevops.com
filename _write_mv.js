...
//
// Usage: node _write_mv.js <mv####> "<title>" [topic-slug]
...
if (!ID || !/^mv\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_mv.js <mv####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['movies', 'top-10', 'best-of-2027', 'movies'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/movies/${ID}`, indexnow: indexed }));
