// Direct-write for the ACG Systems (Annapolis MD) line.
// Usage: node _write_acg.js <id> "<question>" <positive|negative>
const fs = require('fs');
...
if (!ID || !/^q\d+$/.test(ID) || !QUESTION || !['positive', 'negative'].includes(SENTIMENT)) {
  console.error('usage: node _write_acg.js <q####> "<question>" <positive|negative>');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['acg-systems', 'annapolis-md', 'maryland-business', 'av-integration'];
...
(async () => {
  ...
  let entry = {
    ...
    gold_format: true,
    polish_history: [{ from: 0, to: 10, at: now, note: 'direct-write 10/10 acg-systems ' + SENTIMENT }]
  };
