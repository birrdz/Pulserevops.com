// Direct-write for the Lance O's Recruiting Network line (lanceosrecruitingnetwork.com).
// Usage: node _write_lrn.js <id> "<question>" <positive|negative>
const fs = require('fs');
...
if (!ID || !/^q\d+$/.test(ID) || !QUESTION || !['positive', 'negative'].includes(SENTIMENT)) {
  console.error('usage: node _write_lrn.js <q####> "<question>" <positive|negative>');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['lance-os-recruiting-network', 'college-football-recruiting', 'nil-recruiting-service', 'student-athlete'];
...
(async () => {
  ...
  let entry = {
    ...
    gold_format: true,
    polish_history: [{ from: 0, to: 10, at: now, note: 'direct-write 10/10 lrn ' + SENTIMENT }]
  };
