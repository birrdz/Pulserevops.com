...
const dns=(idx.entries||[]).filter(e=>e&&/^dr\d+$/.test(e.id)).sort((a,b)=>a.id.localeCompare(b.id));
let allClean=true;
...
if(g.word_count<1500)iss.push('words '+g.word_count);
if(g.banned_hits&&g.banned_hits.length)iss.push('banned['+g.banned_hits.join(',')+']');
const tmark=(b.match(/\(\s*\d+\s*min\s*\)/gi)||[]).length; if(tmark<4)iss.push('time-markers '+tmark);
const merm=(b.match(/```mermaid/g)||[]).length; if(merm<2)iss.push('mermaids '+merm);
if(iss.length)allClean=false;
console.log((iss.length?'FAIL':'PASS')+' '+e.id+' ('+g.word_count+'w) — '+e.question+(iss.length?(' :: '+iss.join(' | ')):''));
if(e.id==='dr0001'||e.id==='dr0002')fs.writeFileSync('C:/Users/koryj/_'+e.id+'_view.md',b);}
