...
const sps=(idx.entries||[]).filter(e=>e&&/^sp\d+$/.test(e.id)).sort((a,b)=>a.id.localeCompare(b.id));
let allClean=true;
for(const e of sps){const rec=await s.get('answers/'+e.id+'.json',{type:'json'});const b=rec.answer;const g=gradeEntry(e.id,b);const iss=[];
if(g.score<11)iss.push('score '+g.score+'/12 ['+g.missing.join(',')+']');
if(g.word_count<700)iss.push('words '+g.word_count);
if(g.banned_hits&&g.banned_hits.length)iss.push('banned['+g.banned_hits.join(',')+']');
const hasBody=/<blockquote/i.test(b)||/^>\s+/m.test(b)||(b.match(/\[[^\]\n]{1,40}\]/g)||[]).length>=2;
if(!hasBody)iss.push('no-speech-body');
if(iss.length)allClean=false;
console.log((iss.length?'FAIL':'PASS')+' '+e.id+' ('+g.word_count+'w) — '+e.question+(iss.length?(' :: '+iss.join(' | ')):''));
if(e.id==='sp0001'||e.id==='sp0002')fs.writeFileSync('C:/Users/koryj/_'+e.id+'_view.md',b);}
