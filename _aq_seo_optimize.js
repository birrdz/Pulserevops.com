// SEO-optimize all live aq#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _aq_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  aqRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of aqRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: AQ_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/aquariums/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(AQ_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: AQ_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: AQ_STATIC_URLS,
  };
  fs.writeFileSync('C:/Users/koryj/website/_aq_seo_optimize_report.json', JSON.stringify(report, null, 1));
