// SEO-optimize all live rs#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _rs_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const rsRows = (idx.entries || []).filter((e) => e && /^rs\d+$/i.test(e.id));
  rsRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of rsRows) {
    ...
    const displayChanged = JSON.stringify(entry.seo_brand_keywords || []) !== JSON.stringify(newDisplay);
    const faqChanged = JSON.stringify(entry.seo_semantic_faq_questions || []) !== JSON.stringify(newFaq);
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: RS_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/resorts/${id}`,
      indexnow: indexnow ? { ok: indexnow.ok, pings: indexnow.pings } : null,
    });
    if (changed) console.log(`${id}\ttags=${newTags.length}\tkw=${newDisplay.length}\tindexnow=${indexnow?.ok ?? 'skip'}`);
  }

  if (!DRY) await store.setJSON('_index.json', idx);

  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(RS_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: RS_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: RS_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_rs_seo_optimize_report.json', JSON.stringify(report, null, 1));
