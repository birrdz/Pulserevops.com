...
//
// Usage: node _write_ga.js <ga####> "<title>" [topic-slug]
...
if (!ID || !/^ga\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_ga.js <ga####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['gatherings', 'top-10', 'best-of-2027', 'gatherings'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/gatherings/${ID}`, indexnow: indexed }));
