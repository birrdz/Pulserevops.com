// Direct-write for the Electronic Reviews pillar — er####.
// Top-10 product rankings with Best Overall + Best Value highlights.
// Usage: node _write_er.js <er####> "<title>" [topic-slug]
...
if (!ID || !/^er\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_er.js <er####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['electronic-review', 'top-10', 'product-review', 'consumer-electronics', 'best-of-2027'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade electronic-review' : 'direct-write 10/10 electronic-review' },
    ],
  };
