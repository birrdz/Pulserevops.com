// Generate + publish st0499–st0798 (300 sales training Top-10 entries).
// Usage: node _st_sprint300_run.js [startId] [--dry-run]
// Image pipeline: DDG-slow (sequential via _write_st.js) — expect ~2–5 min/entry for Top-10 covers + product cards.
const fs = require('fs');
const { execSync } = require('child_process');
const { buildBody } = require('./_st_sprint300_bodies');
const { createBatchProgressReporter } = require('./_progress_email');

const QUEUE = JSON.parse(fs.readFileSync('C:/Users/koryj/website/_st_sprint300.json', 'utf8'));
const PROG = 'C:/Users/koryj/website/_st_sprint300_progress.json';
const LOG = 'C:/Users/koryj/website/_st_sprint300_run.log';
const DRY = process.argv.includes('--dry-run');
const startArg = process.argv.find((a) => /^st\d+$/i.test(a));
...
function publish(id, title, slug, body) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const out = execSync(`node _write_st.js ${id} "${titleEsc}" ${slug}`, {
      ...
      if (!/Image LAW|product_imgs/i.test(msg) || attempt >= 2) throw e;
      log(`RETRY ${id} image law attempt ${attempt + 2}`);
    }
  }
  throw lastErr;
}
...
(async () => {
  const progress = createBatchProgressReporter({
    label: 'PULSE st sprint300',
    total: items.length,
    interval: 10,
    pillarUrl: 'https://pulserevops.com/sales-trainings',
  });
  await progress.start(`<p>Publishing <b>${items.length}</b> sales training entries (${items[0]?.id || 'none'} → ${items[items.length - 1]?.id || 'none'})</p>`);
  ...
  await progress.complete(`<p>Published <b>${pub}</b>, rejected <b>${rej}</b>. Run <code>node _st_finish.js</code> for SEO hub sync + verify + deploy.</p>`);
