...
(async()=>{
  ...
  async function worker(){
    while(cursor<weak.length){
      ...
      if(Date.now()-lastEmail>15*60*1000){lastEmail=Date.now();await emailOwner(`PULSE er DDG backfill: ${fixed}/${weak.length}`,`<p>Electronic Reviews image backfill (keyless/DuckDuckGo) in progress: <b>${fixed}</b>/${weak.length} entries updated, ${improved} improved, ${addedImgs} images added.</p>`);}
    }
  }
  await Promise.all(Array.from({length:CONCURRENCY},worker));
  console.log(`\nDDG DONE. updated=${fixed} improved=${improved} imagesAdded=${addedImgs}`);
  fs.writeFileSync('C:/Users/koryj/website/_er_ddg_result.json',JSON.stringify({targets:weak.length,fixed,improved,addedImgs},null,1));
  await emailOwner(`PULSE er DDG backfill COMPLETE`,`<p>Electronic Reviews keyless image backfill finished. ${improved} entries improved, ${addedImgs} real images added across ${weak.length} targeted entries.</p><p>https://pulserevops.com/electronic-reviews</p>`);
