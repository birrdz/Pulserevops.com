...
//
// Usage: node _write_gm.js <gm####> "<title>" [topic-slug]
...
if (!ID || !/^gm\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_gm.js <gm####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['gaming', 'top-10', 'best-of-2027', 'gaming'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/gaming/${ID}`, indexnow: indexed }));
