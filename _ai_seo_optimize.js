// SEO-optimize all live ai#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _ai_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const aiRows = (idx.entries || []).filter((e) => e && /^ai\d+$/i.test(e.id));
  aiRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of aiRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: AI_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/ai-infrastructure/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(AI_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: AI_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: AI_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_ai_seo_optimize_report.json', JSON.stringify(report, null, 1));
