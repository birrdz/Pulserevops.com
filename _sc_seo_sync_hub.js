// Sync Pulse Schools hub meta + JSON-LD. Usage: node _sc_seo_sync_hub.js
const fs = require('fs');
const path = require('path');
const { SC_HUB_PAGE, SC_HUB_URL, hubMetaKeywords, hubFaqJsonLd, hubKeywordItemListJsonLd, SC_KEYWORD_PHRASES } = require('./_sc_compete_semantic_keywords');
const { pingIndexNowUrlList } = require('./netlify/functions/lib/indexnow-ping-entry');

const HUB = path.join(__dirname, SC_HUB_PAGE);
...
const collectionLd = {
  '@context': 'https://schema.org', '@type': 'CollectionPage', '@id': `${SC_HUB_URL}#webpage`,
  name: 'Pulse Schools', url: SC_HUB_URL, description: "Top-10 school and college rankings with Best Overall + Best Value picks.",
  keywords: phrases.slice(0, 24).join(', '),
};
const ldBlock = `<script type="application/ld+json">\n${JSON.stringify(collectionLd, null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubFaqJsonLd(), null, 2)}\n</script>\n<script type="application/ld+json">\n${JSON.stringify(hubKeywordItemListJsonLd(SC_HUB_URL), null, 2)}\n</script>`;
...
console.log('hub synced', SC_HUB_PAGE, 'phrases', SC_KEYWORD_PHRASES.length);
(async () => { const ping = await pingIndexNowUrlList([SC_HUB_URL]); console.log('hub IndexNow', JSON.stringify(ping)); })().catch(console.error);
