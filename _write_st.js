// Direct-write for the Sales Trainings pillar — st####.
// st0001–st0498: 60-min facilitator trainings. st0499+: Top-10 rankings (Best Overall + Best Value).
// Usage: node _write_st.js <st####> "<title>" [topic-slug]
...
if (!ID || !/^st\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_st.js <st####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;
const IS_TOP10 = parseInt(ID.slice(2), 10) >= 499;

const BASE_TAGS = IS_TOP10
  ? ['sales-training', 'top-10', 'sales-enablement', 'sales-coaching', 'best-of-2027']
  : ['sales-training', 'sales-meeting', 'pulse-training', 'sales-enablement', 'sales-coaching'];
  ...
  let entry = {
    ...
    polish_history: [
      ...(existing && Array.isArray(existing.polish_history) ? existing.polish_history : []),
      {
        ...
        note: isUpgrade
          ? IS_TOP10
            ? 'gold-format upgrade sales-training top-10'
            : 'gold-format upgrade sales-training'
          : IS_TOP10
            ? 'direct-write 10/10 sales-training top-10'
            : 'direct-write 10/10 sales-training 60min',
      },
    ],
  };
