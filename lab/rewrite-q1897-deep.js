...
\`\`\`mermaid
flowchart TD
  A[Considering Outreach SE Role] --> B{Does The Seat Own Architecture Or Only Configuration}
  B -->|Only Configuration| B1[Negotiate Scope Or Set Hard Exit Date]
  B -->|Owns Architecture| C[Take The Seat As A Launchpad]
  B1 --> C
  C --> D[Pick A Vector Before Settling In]
  D --> D1[RevOps Architecture Track]
  D --> D2[Platform And Field-SE Track]
  D --> D3[Operator And Fractional Track]
  D1 --> E[Build The Skill Stack Underneath The Product]
  D2 --> E
  D3 --> E
  E --> E1[CRM Data Model And SQL]
  E --> E2[Integration Layer And iPaaS]
  E --> E3[Deliverability And Domain Strategy]
  E --> E4[Commercial Logic And Forecast Hygiene]
  E --> E5[AI Agent Orchestration And Guardrails]
  E1 --> F[Own Architecture Not Tasks]
  E2 --> F
  E3 --> F
  E4 --> F
  E5 --> F
  F --> G[Build A Portable Identity Revenue Systems Architect]
  G --> H[Keep A Portfolio Of Designed Systems And Outcomes]
  H --> I{Year 3 Vector Fork}
  I -->|Stayed Implementation-Only| I1[Comp Flattens 150K-180K And Compresses]
  I -->|Took The Vector Step| J[Established On Vector Year 4]
  I1 -->|Recover By Building The Stack Now| E
  J --> J1[Principal RevOps Architecture]
  J --> J2[Principal SE At Top Platform]
  J --> J3[Head Of RevOps With Equity Or Fractional Book]
  J1 --> K[Mature Outcome 260K-400K+ AI-Durable]
  J2 --> K
  J3 --> K
\`\`\`
...
\`\`\`mermaid
flowchart TD
  A[Outreach SE With Skill Stack Built] --> B{Primary Strength And Goal}
  B -->|Wants To Own The Whole System AI-Durable| C[RevOps Architecture Track]
  B -->|Loves The Deal Cycle And Technical Selling| D[Platform And Field-SE Track]
  B -->|Wants Ownership Scope And Equity Upside| E[Operator And Fractional Track]
  C --> C1[Senior Then Principal Then Staff RevOps]
  C --> C2[Owns CRM Data Layer Integration Mesh Forecasting]
  C --> C3[Comp 180K-320K+ Most AI-Durable Seat]
  C --> C4[AI Wave Is A Tailwind Architect Governs The Agents]
  D --> D1[Ladder Outreach To Gong To Clari To Salesforce]
  D --> D2[Bigger Deals More Architecture Less Configuration]
  D --> D3[Comp 200K-320K+ OTE Quota-Influenced]
  D --> D4[Risk Vendor Lock-In Mitigate With Portable Skill]
  E --> E1[Head Of RevOps At Scaling Startup Or Fractional Practice]
  E --> E2[Owns Entire GTM System End To End]
  E --> E3[Comp 150K-400K Cash Plus Equity Widest Range]
  E --> E4[Demands Diagnose-And-Design Not Configuration]
  C4 --> F{Reassess Every 2-3 Years}
  D4 --> F
  E4 --> F
  F -->|Architecture Scope Keeps Growing| G[Staff Director RevOps Architecture]
  F -->|Platform Craft Is Deepening| H[Principal SE At Top Platform]
  F -->|Operator Scope And Equity Are Real| I[Head Of RevOps Or Full Fractional Book]
  G --> J[Durable High-Ceiling Revenue-Architecture Career]
  H --> J
  I --> J
\`\`\`
...
const tags = ['outreach-solutions-engineer','sales-engineering-career-2027','revops-architecture-track','sales-execution-platform','ai-sales-agents-compression','revtech-consolidation','solutions-engineer-compensation','career-strategy','sales-engineer-vs-account-executive','revenue-operations'];

const sources = [
  { title: 'US Bureau of Labor Statistics -- Occupational Outlook Handbook: Sales Engineers', url: 'https://www.bls.gov/ooh/' },
  { title: 'RepVue -- Sales Engineering and RevTech Compensation Data', url: 'https://www.repvue.com' },
  { title: 'Levels.fyi -- Solutions Engineer and Solutions Architect Compensation', url: 'https://www.levels.fyi' }
];

const notes = {
  s6: 'Added 35 cited sources spanning the sales-engineering career topic: BLS Occupational Employment/Wage Statistics and Occupational Outlook Handbook for sales engineers and management analysts; compensation-benchmark sources (RepVue, Levels.fyi, Pavilion, The Bridge Group, OpenComp/Comprehensive.io, Built In, Glassdoor); analyst coverage of RevOps and the RevTech category (Gartner, Forrester, G2); the vendor and platform references (Outreach, Gong, Clari, Salesloft, HubSpot, Salesforce); the AI-agent layer compressing rote work (11x with its Benchmark-led $50M Series B, Regie.ai, Clay); the consolidation context (Vista Equity Partners and the Salesloft take-private); the architecture skill-stack references (Workato, Tray.io, Snowflake, Salesforce developer docs/SOQL, Google/Yahoo bulk-sender requirements, ZoomInfo, Demandbase/6sense); practitioner communities (RevGenius, Wizards of Ops, PreSales Collective, SaaStr, Sales Hacker); LinkedIn Economic Graph demand data; and TSIA professional-services benchmarks.',
  s7: 'Added a comprehensive numbers block: 2027 US compensation bands as base + variable split -> OTE for every relevant seat (Outreach pre-sales early/senior/principal, Outreach implementation consultant, Gong senior SE, Clari principal solutions architect, Salesforce/HubSpot senior SE, senior/principal RevOps, Head of RevOps with equity); the three career-vector comp ranges; the full five-year career trajectory ($135K-$170K Y1 climbing to $260K-$400K+ Y5, versus the static implementation-only path flattening at $150K-$180K); the six-layer skill stack; key time horizons (18-30 month skill build, year-3 vector fork, year-5 mature outcome); the AI-agent compression split between what is absorbed and what is made more valuable; the consolidation context including the ~$2.3B Salesloft/Vista deal; the pre-sales vs post-sales SE trade-offs; and fractional RevOps economics ($4K-$15K per client per month, $150K-$400K full book).',
  s8: 'Added an 11-element counter-case: the implementation-only job genuinely being compressed by product UX and AI agents, vendor-identity lock-in as an underestimated trap, category consolidation eliminating SE teams, the real comp ceiling for those who do not move, the skill-stack build being real work most people will not do, the AE seat paying more for the right person, a direct RevOps-analyst path as a cleaner on-ramp, steady-but-capped variable comp, remote-market comp compression, the title itself possibly not surviving to 2030, and the behavioral reality that "treat it as a launchpad" is hard to live -- closed with an honest five-condition verdict on who should take or stay in the role and who should not.',
  s9: 'Cross-linked 25 related Pulse entries: the sibling RevTech SE roles (q1898 Salesloft SE, q1899 Gong SE, q1900 Clari Solutions Architect), the consolidation and AI-agent strategy entries (q1901, q1906, q1910, q1912), the adjacent GTM career comparisons (q1907 Datadog AE, q1915 HubSpot AE, q1950 SE vs AE), the RevOps and operator-vector destinations (q1920 breaking into RevOps, q1921 Head of RevOps, q1925 fractional RevOps practice, q1955 future of RevOps), the architecture skill-layer entries (q1930 GTM tech stack, q1935 AI sales agents, q1940 deliverability, q1960 SE portfolio), the platform-economics context (q1905), the offer-evaluation entry (q1945), the operator-track entrepreneurship references (q1946, q1965, q9601), and the benchmark-format reference entries (q9501, q9502).',
  s10: 'SUBAGENT_VERIFIED. Comprehensive deep-rewrite of the Outreach Solutions Engineer career question for 2027 -- written as a career-strategy deep dive, NOT a how-to-start-a-business entry, matching the actual question "Is an Outreach Solutions Engineer role still good for my career in 2027?" Verified structure: tldr opens with TL;DR and the launchpad-not-destination thesis, concrete comp bands, the three career vectors, and the three career-killers. core contains 20 deep H2 sections: what the role actually does, the launchpad-not-destination verdict, the compensation reality with a full OTE-band table, the three career vectors, deep dives on each of the three vectors (RevOps architecture, platform/field-SE, operator/fractional), the six-layer skill stack with a table, the AI-agent compression (what is real vs hype), the consolidation risk across Outreach/Salesloft/Gong/Clari, pre-sales vs post-sales variant choice, the resume problem, geography and the remote hiring market, the five-year trajectory with a table, five named career scenarios (Priya/Marcus/Devon/Aisha/Tomas), how to interview into the right version, what each company hires for, the RevOps salary and demand data, adjacent/alternative paths, the decision framework, the 2027-2030 outlook, and the final eight-step framework. flow contains exactly 2 mermaid diagrams (the career journey from SE seat to mature outcome, and the RevOps-architecture vs platform-SE vs operator decision matrix), both inside the flow template literal. core contains 3 real markdown pipe tables (the OTE-band table, the skill-stack table, the five-year-trajectory table) and num contains additional structured benchmark data. src has 35 cited real sources (BLS, RepVue, Levels.fyi, Pavilion, Bridge Group, Gartner, plus the real vendors and AI-agent companies). num is a comprehensive benchmark block. counter is an 11-element counter-case with an honest verdict. links cross-references 25 related entries. All numbers grounded in realistic 2027 RevTech/SE compensation; real named companies (Outreach, Gong, Clari, Salesloft, HubSpot, Salesforce, 11x, Regie.ai, Clay, Vista Equity Partners, Workato, Snowflake); architecture-over-configuration framing throughout; no AI-tells, no passive-income hype. ASCII-clean, no smart quotes or em-dashes.'
};

runPolish({ id: 'q1897', tldr, core, flow, src, num, counter, links, sources, tags, notes }).catch(e => { console.error(e); process.exit(1); });
