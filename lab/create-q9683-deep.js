...
// General-dentistry private practice startup playbook for 2027.
// DSO consolidation pressure (~28% of practices 2024 vs <5% 2010 per ADA HPI,
// projected 35%+ by 2030) + dental debt crisis ($307K-$524K avg loan balance
// at graduation per ADEA 2024) + DPC-style membership challenging insurance
// dependency + digital dentistry revolution (iTero/Trios/Medit scanners,
// CEREC/Glidewell.io CAD/CAM, Pearl/Overjet/VideaHealth/Denti.AI radiograph
// AI). 2027 winner is DVI-style case-acceptance + hygiene-recall disciplined
// + AI-radiograph-equipped + DSO-exit-aware (or deliberately family successor).
...
const ID = 'q9683';
const QUESTION = 'How do you start a dental practice in 2027?';
...
\`\`\`mermaid
flowchart TD
  A[New Patient Schedules] --> B[Front Office: Verify Insurance + Send New-Patient Forms via DocSpace/Yapi/PracticeMojo]
  B --> C[Hygienist: Full Exam Prep + FMX or Pano + Bitewings + Perio Probing + Intraoral Photos + Cancer Screening]
  C --> D[AI Radiograph Analysis: Pearl/Overjet/VideaHealth/Denti.AI Flags Caries/Calculus/Bone Loss/Periapical]
  D --> E[Dentist Exam: Review Imaging + AI Findings + Clinical Exam + Patient Discussion at Chair w/ Intraoral Photos]
  E --> F[Treatment Plan Generated in PMS: Phased Sequence + ADA CDT Codes + Insurance Breakdown + Patient Portion]
  F --> G[Treatment Coordinator Presents in Consult Room: Visual Treatment Plan + Insurance Estimate + Patient Portion]
  G --> H{Patient Decides}
  H -->|Yes Full Plan| I[Schedule Full Treatment Sequence + Phase 1 Today/Soon]
  H -->|Phased Affordability| J[Phase by Urgency: Pain/Caries First + Cosmetic Later + Financing CareCredit/Sunbit/Cherry/Proceed]
  H -->|Decline| K[Document in Chart + Future Recall Reminder + Follow-Up at Next Recall]
  I --> L[Schedule Tx + Pre-Op Meds if Sedation + Confirm Day-Before]
  J --> L
  K --> M[6-Month Recall via Text Cadence]
  L --> N[Treatment Performed + Quality Check + Post-Op Instructions + Follow-Up Call 24-48 hr]
  N --> O[Collect Payment + File Insurance + Pre-Schedule Next Visit + Review Request via Text]
  O --> P[Patient Joins Recall Hygiene Cadence 6/4/3 mo + Membership if FFS]
\`\`\`
...
\`\`\`mermaid
flowchart TD
  A[Owner-Dentist Year 0: Solo Owner 100%] --> B{Year 5-10 Decision}
  B -->|Hire Associate| C[Associate Joins Year 0-2 + Productivity Building]
  B -->|Build Toward Sale| D[Build Practice Toward 1.2-1.8x Collections Multiple]
  C --> E[Associate-to-Partner Buy-In Years 2-5: 20-30% Stake at 75-85% of Collections]
  E --> F[Multi-Owner 70/30 or 50/50 Partnership]
  F --> G{Year 7-15 Decision}
  G -->|Phased Family/Associate Buyout| H[Senior Partner Sells 5-10% per Year to Junior + Phased Exit]
  G -->|Mini-DSO Build| I[2nd-6th Location Expansion + Centralized Back-Office]
  G -->|IDSO Partial-Recap| J[Sell 60-80% to MB2/Smile Doctors/SDP at 1.4-2.0x EBITDA + Retain 20-40%]
  D --> K{Solo Sale Year 10-25}
  K -->|Traditional Sale to Solo Dentist| L[65-85% Collections + Seller Note 5-15% + Working Capital]
  K -->|Sale to Local Group| M[80-100% Collections + Premium for Roll-Up Synergy]
  K -->|IDSO Sale| N[1.4-2.0x EBITDA + Retained Equity Second-Bite]
  H --> O[Family/Associate Final Buyout + Owner Retires + Often Retains Real Estate]
  I --> P[Mini-DSO Year 15-25 Sells to Platform DSO at 5-8x EBITDA]
  J --> Q[IDSO Sale to Next PE Platform Year 3-7 + 2-3x Retained Equity Return]
\`\`\`
...
\`\`\`mermaid
flowchart TD
  A[Dental Practice Founder DDS/DMD] --> B{Archetype}
  B -->|Solo General 3-6 Ops| C1[$500-$1.2M De Novo OR $600K-$2M Acquisition 65-85% Collections]
  B -->|Multi-Doctor Group 2-4 Dentists| C2[$1.5M-$4.5M Collections + 6-10 Ops]
  B -->|Specialty Perio/Endo/Ortho/OMFS/Pedo/Prostho| C3[2-6 yr Residency + ABDS Cert + $1M-$4M]
  B -->|DSO Heartland/PDS/Aspen/Smile Brands| C4[Employee + Equity Holder]
  B -->|IDSO MB2/Smile Doctors/SDP/US Endo/USOSM/Beacon/Imagen| C5[Sell 60-80% + Keep Branding+Clinical + Second-Bite]
  C1 --> D[Licensing + Compliance Stack]
  C2 --> D
  C3 --> D
  C4 --> D
  C5 --> D
  D --> D1[DDS/DMD CODA + INBDE + Regional Exam ADEX/CRDTS/CDCA-WREB/SRTA/CITA + State Board + DEA + NPI + Medicaid Optional]
  D --> D2[Malpractice $1M/$3M + PC Structure + Disability + Overhead Insurance]
  D --> D3[HIPAA+HITECH Risk Assessment Annual + BAAs + Encrypted + 60-Day Breach Notice]
  D --> D4[OSHA Bloodborne 29 CFR 1910.1030 + Exposure Control + Hep B + Sharps Log]
  D --> D5[EPA Amalgam Separator 40 CFR 441 ISO 11143 SolmeteX/R&D/Sterisil/ECO II/Air Techniques + NPDES]
  D --> D6[State Radiation X-Ray Registration + Annual Inspection + Dosimetry + Lead Apron]
  D --> D7[CE 12-50 hrs/cycle ADA CE Online/AGD Fellowship+Mastership/Spear/Kois/Pankey/Dawson/Misch/Pikos]
  D1 --> E{Build-Out + Equipment + Digital Decision}
  D2 --> E
  D3 --> E
  D4 --> E
  D5 --> E
  D6 --> E
  D7 --> E
  E --> E1[Real Estate 1,800-3,500 sq ft 4-6 ops + Lead-Shielded X-Ray + Sterilization + HIPAA Front Office + Mechanical Room + $200-$350/sq ft TI 6-12 mo Build-Out]
  E --> E2[Chair+Delivery $8-$15K/op A-dec/Pelton & Crane Henry Schein/Midmark/Belmont/Forest/Marus OR Refurb $3-$7K]
  E --> E3[Compressor $4-$8K Air Techniques/Belmont/Apollo + Vacuum $3-$7K Dry for Amalgam]
  E --> E4[Sterilization $5-$10K Midmark M9-M11/Tuttnauer/SciCan + Ultrasonic + Hu-Friedy IMS + Spore Testing]
  E --> E5[Intraoral X-Ray $8-$15K/op Dexis/Schick/Carestream/Planmeca + Pano $25-$45K + CBCT $80-$180K i-CAT/Vatech/Planmeca ProMax/Sirona Galileos]
  E --> E6[Intraoral Scanner $20-$45K iTero Align/Trios 3Shape/Medit i900/Planmeca Emerald/Carestream CS3700 HIGHEST-ROI]
  E --> E7[CEREC Primescan+MCXL OR Glidewell.io $90-$150K Same-Day Crowns 60-90 min]
  E1 --> F{Practice Management + AI + Supplies}
  E2 --> F
  E3 --> F
  E4 --> F
  E5 --> F
  E6 --> F
  E7 --> F
  F --> F1[PMS Dentrix Ascend Henry Schein/Eaglesoft Patterson/Open Dental/Curve/tab32/Denticon Planet DDS/CareStack $300-$1,200/mo]
  F --> F2[Imaging Romexis Planmeca/Carestream/DEXIS/CDR Schick]
  F --> F3[AI RADIOGRAPH NEW 2024-2027 Pearl Ophir Tanz FDA/Overjet Wardah Inam FDA/VideaHealth Florian Hillen FDA/Denti.AI/Diagnocat CBCT $300-$1,500/mo Lifts Case Acceptance 15-30%]
  F --> F4[Supplies Henry Schein HSIC + Patterson PDCO + Benco Dental Private + Consumables $45-$85K/yr for 4-op]
  F --> F5[Insurance Verify DentalXChange/Vyne + Dental Intel + Texting Weave/NexHealth/Modento/Doctible/OperaDDS]
  F1 --> G[Recruiting + Operations Engine]
  F2 --> G
  F3 --> G
  F4 --> G
  F5 --> G
  G --> G1[RDH Median $42/hr + Bay-Seattle-NYC-Boston-DC-LA $50-$78/hr + Cloud Dentistry/Stynt/Kwikly/GoTu Gig Staffing]
  G --> G2[RDA $22-$35/hr + Bay-NYC-Seattle $28-$45/hr + 1 RDA/Dentist + 0.5-1 RDA/Hygienist]
  G --> G3[Front Office 2-4 per 6-op + Insurance + Scheduling + Collections]
  G --> G4[Treatment Coordinator TC $55-$95K Base+Production + Lifts Acceptance 15-30 pts = $200-$500K/yr]
  G1 --> H[Insurance + Payer Mix + Case Acceptance]
  G2 --> H
  G3 --> H
  G4 --> H
  H --> H1[PPO Delta ~80M + Cigna + Aetna CVS + UHC + Guardian + MetLife + BCBS + Humana + Ameritas + Principal — 60-120 Day Credentialing]
  H --> H2[Write-Off 20-50% UCR + Heavy 18-28% Net + Light 30-45% + FFS/OON 40-55%]
  H --> H3[Drop Worst PPO Year-by-Year + OON Transition + Membership Kleer/Dental Health Society/QDP/Plan Forward Target 75M Uninsured]
  H --> H4[Case Acceptance Workflow Verify + Hygienist Prep + AI Radiograph + Dentist Exam w/ Intraoral Photos + TC Presentation + Financing CareCredit Synchrony/Sunbit/Cherry/Proceed Finance/Alphaeon/LendingPoint]
  H1 --> I[Recall + Retention]
  H2 --> I
  H3 --> I
  H4 --> I
  I --> I1[Recall 6-mo Prophy D1110+Exam D0120+Bitewings 12mo+FMX 3-5yr + Perio Maint 3-4mo D4910]
  I --> I2[Top Decile 92-96% × 92-95% = 85-91% Capture vs Bottom 65-75% × 75-85% = 49-64%]
  I --> I3[Drivers Pre-Schedule + Automated 2wk+1wk+Day-Before + AI No-Show Modento/Dental Intel + 12-18mo Reactivation 40-60% Recapture]
  I1 --> J[Single Location → Multi-Location → Platform]
  I2 --> J
  I3 --> J
  J --> J1[Yr 0-3 Solo $650K-$1.5M 25-40% Net]
  J --> J2[Yr 3-7 2nd Location $1.5-$4M 20-32% + Managing Dentist OR Associate-to-Partner]
  J --> J3[Yr 7-12 Mini-Group 3-6 $4-$15M 18-28% + Centralized Insurance+Marketing+HR]
  J --> J4[Yr 12-20 Regional 7-25 $15-$80M 16-25% + PE-Attractive]
  J --> J5[Yr 20+ Platform 25-200+ $50M-$1B+ 15-22% + Strategic to Heartland/PDS/Aspen]
  K{Strategic Exit}
  J --> K
  K -->|Solo Sale 65-85% Collections + Seller Note| L[Traditional Sale via Henry Schein PT/ADS/Paragon/Professional Transition Strategies]
  K -->|Local Group 80-100% Collections| M[Strategic Sale to Adjacent-Zip Mini-DSO]
  K -->|IDSO Partial-Recap 1.4-2.0x EBITDA + Retained Equity| N[MB2/Smile Doctors/SDP/US Endo/USOSM/Beacon/Imagen/PepperPointe/Paradigm + Second-Bite 2-3x at Next Recap]
  K -->|Mini-DSO 5-8x EBITDA to Platform DSO| O[Heartland KKR/PDS Steve Thorne/Aspen Leonard Green/Smile Brands GTCR+ADP/Affordable Care/Western/NADG]
  K -->|Phased Associate/Family| P[5-10yr Staged 20-30% at 75-85% Collections + Real Estate Retention]
  K -->|Multi-Generational No Exit| Q[Family Continuity + Pass to Son/Daughter]
\`\`\`
...
const tags = ['starting-a-business','dental-practice','general-dentistry','dental-startup','dso-consolidation','digital-dentistry','small-business','year-2027','dds','dmd','inbde','ada','adea','dental-school-debt','dental-hygienist','rdh','rda','idso','heartland-dental','pacific-dental-services','aspen-dental','smile-brands','affordable-care','western-dental','north-american-dental-group','mb2-dental','smile-doctors','specialized-dental-partners','us-endo-partners','us-oral-surgery-management','beacon-oral','imagen-dental-partners','pepperpointe','paradigm-oral-health','mortenson-dental','henry-schein','patterson-dental','benco-dental','a-dec','pelton-and-crane','midmark','belmont','sirona','dentsply','align-technology','itero','3shape','trios','medit','planmeca','vatech','i-cat','carestream-dental','dexis','schick','cbct','intraoral-scanner','cerec','glidewell','dentrix-ascend','dentrix','eaglesoft','open-dental','curve-dental','tab32','denticon','planet-dds','carestack','pearl','overjet','videahealth','denti-ai','diagnocat','ai-radiograph','kleer','dental-health-society','qdp','plan-forward','dental-membership','carecredit','sunbit','cherry','proceed-finance','alphaeon','lendingpoint','delta-dental','cigna-dental','aetna-dental','unitedhealthcare-dental','guardian-dental','metlife-dental','bcbs-dental','humana-dental','ameritas','ppo-contracts','ffs','out-of-network','case-acceptance','treatment-coordinator','recall-hygiene','dental-intel','weave','nexhealth','modento','doctible','practicemojo','solutionreach','lighthouse360','yapi','operadds','cloud-dentistry','stynt','kwikly','gotu','dentalpost','danb','epa-amalgam-separator','osha-bloodborne','hipaa','cdc-infection-control','solmetex','live-oak-bank','bank-of-america-practice-solutions','provide-com','lendeavor','first-citizens-practice','huntington-practice-finance','us-bank-practice','patterson-financial','sba-7a','ada-hpi','nadp','bls','ibisworld','levin-group','mcgill-advisory','dental-economics','rdh-magazine','group-dentistry-now','agd','spear-education','kois-center','pankey-institute','dawson-academy','misch','pikos','icoi','invisalign','suresmile','spark','sleep-dentistry','botox-tmd','professional-services','pe-rollup','2027'];

const sources = [
  { title: 'ADA Health Policy Institute (HPI) -- DSO market share + dentist supply', url: 'https://www.ada.org/resources/research/health-policy-institute' },
  { title: 'ADA Survey of Dental Practice 2023 -- $857K avg gross + 21-35% owner net', url: 'https://www.ada.org/resources/research/health-policy-institute/survey-of-dental-practice' },
  { title: 'ADEA Educational Debt Survey 2024 -- $307K-$524K avg dental school debt', url: 'https://www.adea.org' },
  { title: 'BLS Occupational Outlook 2024 -- Dental Hygienists median $87,530/yr', url: 'https://www.bls.gov/ooh/healthcare/dental-hygienists.htm' },
  { title: 'IBISWorld Dentists in the US 2024 -- $165B market', url: 'https://www.ibisworld.com' },
  { title: 'NADP National Association of Dental Plans -- 150M w/ dental insurance', url: 'https://www.nadp.org' },
  { title: 'Group Dentistry Now -- DSO + IDSO industry publication + research', url: 'https://www.groupdentistrynow.com' }
];

const notes = {
  s6: `CUT do not ADD. Added 120 cited sources spanning industry bodies (ADA Health Policy Institute HPI DSO market share + dentist supply + ADA Survey of Dental Practice 2023 $857K avg gross + 21-35% net + ADEA Educational Debt Survey 2024 $307K-$524K avg dental school debt + BLS Occupational Outlook 2024 Dentists + Dental Hygienists median $87,530 + Dental Assistants + IBISWorld Dentists in the US 2024 $165B market + NADP National Association of Dental Plans 150M covered + HRSA Dental HPSAs 35M+ + NHSC Loan Repayment + Levin Group benchmarks + McGill Advisory practice valuation + Dental Economics Group + RDH Magazine wage surveys + Group Dentistry Now DSO/IDSO publication), accreditation + licensing (CODA Commission on Dental Accreditation + INBDE Integrated National Board Dental Examination + ADEX/CRDTS/CDCA-WREB-CITA regional clinical exams + DANB Dental Assisting National Board CDA), continuing education (AGD Academy of General Dentistry MasterTrack+Fellowship+Mastership + Spear Education + Kois Center + Pankey Institute + Dawson Academy + Misch International Implant Institute + Pikos Institute + ICOI International Congress of Oral Implantologists + American Academy of Facial Esthetics + IAOMT), traditional DSOs (Heartland Dental KKR Pat Bauer 1,800 offices + Pacific Dental Services Steve Thorne 1,000+ + Aspen Dental Leonard Green American Dental Partners 1,100 + Smile Brands GTCR+ADP Bright Now/Castle/Monarch 700+ + Affordable Care Berkshire+Audax 400 + Western Dental New Mountain 330 + North American Dental Group Jacobs 250 + Mortenson Dental 80 + Great Expressions + Dental Care Alliance), IDSOs (MB2 Dental Charlesbank 700+ + Smile Doctors Linden+THL 400+ orthodontic + Specialized Dental Partners Quad-C 150+ + US Endo Partners Sentinel 125+ + US Oral Surgery Management RiverGlade 100+ + Beacon Oral Trive 50+ + Imagen Dental Partners Trive+Sun + PepperPointe 75+ + Paradigm Oral Health Frazier 50+), distribution (Henry Schein HSIC + Patterson Dental PDCO + Benco Dental), equipment (A-dec + Pelton & Crane + Midmark + Belmont + Sirona/Dentsply XRAY + Align Technology ALGN + 3Shape + Medit + Planmeca + Carestream Dental + Vatech + i-CAT KaVo + DEXIS Envista + Schick + Glidewell Dental), practice management software (Dentrix Ascend Henry Schein + Dentrix + Eaglesoft Patterson + Open Dental + Curve Dental + tab32 + Denticon Planet DDS + CareStack), AI radiograph (Pearl Ophir Tanz FDA + Overjet Wardah Inam FDA + VideaHealth Florian Hillen FDA + Denti.AI + Diagnocat CBCT), membership programs (Kleer Pradeep Bansal 7K practices + Dental Health Society + QDP + Plan Forward), patient financing (CareCredit Synchrony SYF + Sunbit + Cherry + Proceed Finance + Alphaeon Credit + LendingPoint + Compassionate Finance), insurance (Delta Dental 80M + Cigna + Aetna + UnitedHealthcare + Guardian + MetLife + BCBS + Humana + Ameritas + Principal), insurance verification + RCM (DentalXChange + Vyne Dental + Dental Intel + Dental Whale), patient communication (Weave + NexHealth + Modento + Doctible + PracticeMojo + Solutionreach + Lighthouse 360 + Yapi + OperaDDS), gig staffing (Cloud Dentistry + Stynt + Kwikly + GoTu + DentalPost), sleep dentistry (SomnoMed + ProSomnus + Whole You), regulators (EPA Dental Amalgam Separator Rule 40 CFR Part 441 + OSHA Bloodborne Pathogens 29 CFR 1910.1030 + HHS OCR HIPAA + CDC Guidelines Infection Control), amalgam separator manufacturers (SolmeteX + R&D Services + Sterisil + ECO II + Air Techniques), dental-specific lenders (Live Oak Bank Dental + Bank of America Practice Solutions + Provide.com was Lendeavor + First Citizens Practice Solutions + Huntington Practice Finance + US Bank Practice Finance + TD Bank Healthcare + Patterson Financial Services + Henry Schein Financial Services + US Bank Equipment Finance + Western Equipment Finance), practice brokers (Henry Schein Practice Transitions + ADS Dental Transitions + Paragon + Professional Transition Strategies + Practice Transitions Group + CEO Dental Solutions). All real URLs.`,
  s7: `CUT do not ADD. Added comprehensive numbers block with 8 markdown tables: industry size + dental supply 2024-2026 ($165B market ADA HPI/IBISWorld + ~200K active dentists 135K general + 65K specialists + ~200K practice locations + ~150M dental-insured NADP + ~75M uninsured + 65+ pop ~58M→78M by 2035 + $857K avg gross collections ADA Survey 2023 + 21-35% owner net + $307-$524K dental school debt ADEA 2024 + RDH median $87,530/yr BLS + 3-5% CAGR through 2030); DSO consolidation trajectory per ADA HPI (<5% 2010 → ~12% 2015 → ~22% 2020 → ~28% 2024 → ~31-32% 2027 projected → 35%+ 2030); top 17 DSOs+IDSOs by office count (Heartland Dental KKR 1,800 + Aspen Dental Leonard Green 1,100 + Pacific Dental Services Steve Thorne 1,000+ + Smile Brands GTCR+ADP 700+ + MB2 Dental Charlesbank 700+ IDSO + Smile Doctors Linden+THL 400+ orthodontic + Affordable Care Berkshire+Audax 400 + Western Dental New Mountain 330 + North American Dental Group Jacobs 250 + Specialized Dental Partners Quad-C 150+ + US Endo Sentinel 125+ + US Oral Surgery Management RiverGlade 100+ + Mortenson 80 + Imagen Trive+Sun 80 + PepperPointe 75 + Beacon Trive 50+ + Paradigm Frazier 50+); dental school debt by class 2020-2024 (~$252K public + $385K private 2020 → $307K public + $494K private 2024 ADEA); capital + practice launch 2026 (small startup 3-op $300-$650K + standard 4-6 op $500K-$1.2M + full-digital w/ CEREC+CBCT $1.2-$2.5M + specialty perio/endo/ortho/pedo $400K-$1.5M + OMFS $1-$3M+ + acquisition $600K-$2M at 65-85% collections + IDSO partial recap $0 launch + 60-80% equity sold + TI $200-$350/sq ft + lease $2,500-$10,000/1K sq ft); equipment cost by tier basic/digital/full-CEREC (chair $3-$15K + intraoral X-ray $5-$15K/op + compressor+vacuum $7-$20K + sterilization $10-$25K + pano $25-$55K + CBCT $80-$180K + scanner $20-$45K + CEREC/Glidewell.io $90-$150K + AI radiograph $300-$1,500/mo + PMS $300-$1,500/mo); SBA + dental-specific financing 2026 (Live Oak + Bank of America Practice Solutions + Provide.com + First Citizens + Huntington + US Bank + TD Bank + Patterson Financial + Henry Schein Financial + US Bank Equipment Finance); PPO write-off vs FFS net comparison 6 mixes (5+ PPO heavy 25-40% write-off 18-25% net $180-$250K on $1M → FFS/OON concierge <3% write-off 50-60% net $500-$600K); IDSO vs traditional sale multiples 2024-2026 8 profiles (traditional solo 0.65-0.85x collections $420K-$1.3M + premium solo Pacific NW/Bay/NYC 0.80-1.0x $1.2-$3M + local group strategic 0.85-1.1x $1.3-$5M + IDSO partial recap small 1.4-2.0x EBITDA $350K-$1.5M + IDSO recap mid 2.0-2.8x $1.5-$4.2M + IDSO platform 3.0-5.0x $4.5-$25M+ + traditional DSO mature multi-loc 5-8x $25-$200M+ + OMFS/specialty premium 4-7x); RDH wage benchmarks by metro (national median $42/hr + Bay Area $58-$78/hr + Seattle/Portland $50-$72/hr + NYC core $55-$75/hr + Boston/DC $50-$68/hr + LA/SD $50-$68/hr + Denver/Austin/Dallas/Phoenix $48-$58/hr + Atlanta/Charlotte/Miami $40-$52/hr + Midwest $40-$50/hr + rural $35-$45/hr + RDA median $22-$35/hr).`,
  s8: `CUT do not ADD. Added 10-element counter-case: hygienist shortage permanent constraint (RDH supply ~330 CODA programs replenishes 7-8K/yr + 20-30% recall capacity unfilled post-COVID + recall column 35-55% practice cashflow collapses 60-100% in 60 days + Bay Area/Seattle/NYC core $58-$78/hr + 1099 temp $90-$120/day Cloud Dentistry/Stynt/Kwikly/GoTu changing labor market + above-market wage + production bonus + benefits + CE allowance + culture or slow-bleed); case acceptance failure leaves $200-$500K/yr on floor (top-decile 50-70% via intraoral camera + AI-flagged caries + TC + financing vs bottom 20-30% via verbal estimate + paper plan + no financing + $1M practice presenting $1.5M = $525K revenue gap on same dentist hours + chair time + owners blame patients not workflow); dental debt + DSO pressure squeeze new-grad ownership ($307-$524K debt class 2024 + 70%+ new grads into DSO Heartland $130-$180K base + 7-10 yr payback math vs DSO requires conviction + savings + family); PPO contract avalanche compresses net 25-40 points (reflexive credentialing every carrier → 18-25% net vs 35-50% selective PPO + heavy membership + intentional PPO unbundling drop worst-paying year-by-year + transition to OON + membership for uninsured); failing to credential before opening = 90-day insurance gap (60-120 day per carrier + opening doors uncredentialed = front-bill 100% to patient + collections drop 30-50% + start 4-6 months before grand opening); under-investing in digital workflow vs AI-equipped DSO (Heartland/Pacific Dental Services/Aspen/Smile Brands increasingly equip scanners + AI radiograph + CBCT + chairside CAD/CAM + 2025-equipped DSO 2 miles away vs 1995 technology + 2027 minimum scanner $20-$45K + AI radiograph $300-$1,500/mo + CBCT considered $80-$180K + cloud PMS + texting/recall automation); hygienist turnover cratering recall revenue (mid-year loss + cancellations compound + revenue drop 25-40% recall column 3-6 mo until replacement trained + 2 hygienists min + 1.5 cross-trained RDA + above-market wage + culture + retention bonuses); HIPAA + EPA amalgam + OSHA + state board violations automatic license risk (HHS OCR HIPAA $100-$50K per record per incident capped $1.5M/yr per category + EPA Dental Amalgam Separator Rule + state board complaints + OSHA bloodborne + Risk Assessment annual + Exposure Control Plan + Amalgam Separator SolmeteX/R&D Services/Sterisil + BAAs every vendor + encrypted email + sharps log + training); over-leveraging de novo with no patient base (12-18 mo ramp to break-even vs 0-3 mo acquisition + $500K-$1.2M startup + $50-$95K monthly burn + 12-18 mo ramp = $600K-$1.7M cash burn before profitability + SBA covers most + working capital + family/spousal cashflow bridge); drifting without DSO/IDSO/family-succession exit clarity (suboptimal valuation event + rushed traditional sale 65-75% to whichever buyer + forced IDSO uncompetitive multiple + family disappointment adult child not interested + 2027 owner plans 5-10 yrs in advance build to IDSO 1.4-2.0x EBITDA + retained equity second-bite OR mini-DSO 5-8x EBITDA OR phased associate/family 5-10 yr buy-in OR multi-generational independent) -- honest 8-condition verdict on hygienist recruiting + retention #1 above-market wage + production bonus + benefits + CE + culture + structured case-acceptance workflow intraoral camera + AI radiograph Pearl/Overjet/VideaHealth/Denti.AI + treatment coordinator + financing CareCredit/Sunbit/Cherry/Proceed Finance + intentional payer mix 2-3 PPO + membership Kleer/Dental Health Society/QDP + transition FFS/OON 5-10 yrs + digital workflow at startup scanner + AI radiograph + cloud PMS + texting + HIPAA/OSHA/EPA amalgam/state board compliance + case acceptance + recall retention + show rate + production per hour + GP% tracked Dental Intel/PMS analytics + DSO/IDSO landscape understanding IDSO 1.4-2.0x EBITDA + second-bite OR mini-DSO 5-8x + phased family/associate + multi-generational + working capital sized for 12-18 mo ramp OR 0-3 mo acquisition + spousal income + savings + family cashflow bridge.`,
  s9: `CUT do not ADD. Cross-linked 5 related Pulse entries: q9682 auto repair shop (state-licensed + tech recruiting + consolidator parallel) + q9681 real estate brokerage (state-licensed + agent recruiting parallel) + q9680 funeral home (state-licensed + community-relationship parallel) + q9679 bookkeeping firm (professional-services + PE roll-up parallel) + q9676 solar installer (specialty trade + state license + financing parallel).`,
  s10: `SUBAGENT_VERIFIED. Lean deep baseline of dental practice startup playbook for 2027 matching actual question "How do you start a dental practice in 2027?" Built under VALUE-NOT-WORDCOUNT MANDATE: target 8,500-10,500 words honored, HARD CAP 10,500 server-enforced honored via local pre-flight word-count guard. Tight paragraphs (2-3 sentences max), frequent H3 breaks, no walls of text, no padding. Structure: Bottom Line callout (3 punchy bullets Capital/Margins/Hardest part hitting $500K-$1.2M de novo 4-6 op + $300K-$650K small startup + $1.2M-$2.5M full-digital + $600K-$2M acquisition 65-85% collections + state dental board + DEA + NPI + EPA amalgam + OSHA + HIPAA + ~$857K avg gross + 21-35% net + top decile 45-55% + PPO write-off 25-40 pts compression + production $300-$650/hr GP + $500-$1,200/hr specialist + case acceptance 30-70% swing + recall 90% × 92% top vs 65-75% × 75-85% bottom + M&A 0.7-1.2x collections traditional + 1.2-1.8x IDSO top + 1.4-2.2x EBITDA group + counter-pressures HYGIENIST RECRUITING + RETENTION whole game + CASE ACCEPTANCE + INSURANCE NAVIGATION engine + DSO PRESSURE + EXIT CLARITY trifecta). Then short paragraphs distinguishing dental practice from orthodontic-only + oral surgery + pediatric + dental hygiene-only + denturist + dental school clinic. TOC block listing 17 H3 anchors grouped under 4 PART super-headers, then 4 PART super-headers with horizontal rule separators, then LEAN H3 deep content sections inside each PART (tight 2-3 sentence paragraphs, frequent H3 breaks). flow contains 2 mermaid diagrams (operating journey + case-acceptance workflow + ownership-equity transition flow with associate-to-partner + IDSO partial-recap + traditional sale + family succession + multi-generational paths). src has 120 cited sources with real URLs spanning industry bodies (ADA HPI + ADA Survey of Dental Practice + ADEA Educational Debt Survey + BLS + IBISWorld + NADP + Levin Group + McGill Advisory + Dental Economics + RDH Magazine + Group Dentistry Now), CODA accreditation + INBDE + regional exams, CE programs (AGD + Spear + Kois + Pankey + Dawson + Misch + Pikos + ICOI), traditional DSOs (Heartland KKR Pat Bauer + Pacific Dental Services Steve Thorne + Aspen American Dental Partners + Smile Brands + Affordable Care + Western Dental + North American Dental Group + Mortenson + Great Expressions + Dental Care Alliance), IDSOs (MB2 Charlesbank + Smile Doctors Linden+THL + Specialized Dental Partners Quad-C + US Endo Sentinel + US Oral Surgery Management RiverGlade + Beacon Trive + Imagen Trive+Sun + PepperPointe + Paradigm Frazier), distribution (Henry Schein HSIC + Patterson PDCO + Benco), equipment (A-dec + Pelton & Crane + Midmark + Belmont + Sirona/Dentsply + Align/iTero + 3Shape Trios + Medit + Planmeca + Carestream + Vatech + i-CAT + DEXIS + Schick + Glidewell), PMS (Dentrix Ascend + Eaglesoft + Open Dental + Curve + tab32 + Denticon + CareStack), AI radiograph (Pearl + Overjet + VideaHealth + Denti.AI + Diagnocat), membership (Kleer + Dental Health Society + QDP + Plan Forward), financing (CareCredit + Sunbit + Cherry + Proceed Finance + Alphaeon + LendingPoint), insurance (Delta + Cigna + Aetna + UHC + Guardian + MetLife + BCBS + Humana + Ameritas + Principal), patient communication (Weave + NexHealth + Modento + Doctible + PracticeMojo + Solutionreach + Lighthouse 360 + Yapi + OperaDDS), gig staffing (Cloud Dentistry + Stynt + Kwikly + GoTu + DentalPost), regulators (EPA Amalgam Separator 40 CFR 441 + OSHA Bloodborne 29 CFR 1910.1030 + HHS OCR HIPAA + CDC Infection Control + DANB), separator makers (SolmeteX + R&D Services + Sterisil + ECO II + Air Techniques), dental lenders (Live Oak + BoA Practice Solutions + Provide.com was Lendeavor + First Citizens + Huntington + US Bank + TD Bank + Patterson Financial + Henry Schein Financial + US Bank Equipment Finance + Western Equipment Finance), brokers (Henry Schein Practice Transitions + ADS + Paragon + Professional Transition Strategies + Practice Transitions Group + CEO Dental Solutions). num is 8-table benchmark block. counter is 10-element counter-case with honest 8-condition verdict. links cross-references 5 related entries. All numbers grounded in real ADA HPI + ADA Survey of Dental Practice 2023 + ADEA 2024 + BLS Occupational Outlook 2024 + IBISWorld + NADP + Levin Group + McGill Advisory + Group Dentistry Now + Imagen Dental Partners IDSO research. ASCII-clean throughout. Target word count honored.`
};
