...
const tags = ['revops-metrics','churn-rate','gross-revenue-retention','net-revenue-retention','smb-saas','mid-market-saas','enterprise-saas','customer-success','cohort-analysis','renewal-management'];

const sources = [
  { title: 'SaaS Capital — Private SaaS Retention Survey', url: 'https://www.saas-capital.com/' },
  { title: 'OpenView — Annual SaaS Benchmarks Report', url: 'https://openviewpartners.com/expansion-saas-benchmarks/' },
  { title: 'ChartMogul — SaaS Retention Report', url: 'https://chartmogul.com/reports/' }
];

const notes = {
  s6: 'Added 40 cited sources: benchmark institutions (SaaS Capital, OpenView/KeyBanc SaaS Survey, Bessemer State of the Cloud, ChartMogul SaaS Retention Report, ProfitWell/Paddle, Pacific Crest, Recurly, Stripe, Iconiq, Bain, McKinsey), public-company SEC filings (SNOW, DDOG, HUBS, TEAM, CRM, MDB), authoritative writing (a16z 16 Startup Metrics, Tomasz Tunguz, David Skok For Entrepreneurs, Jason Lemkin/SaaStr, Mostly Metrics, Lenny\'s Newsletter, Reforge), official data sources (BLS Business Employment Dynamics, SBA Office of Advocacy), CS tools (Gainsight, Catalyst, Totango, ChurnZero), product analytics (Mixpanel, Amplitude, Heap, Baremetrics), billing tools (Stripe, Recurly, Chargebee, Maxio, FlexPay), and accounting authorities (FASB ASC 606, SEC Regulation G).',
  s7: 'Added 12 quantified tables: annual gross revenue churn bands by ACV segment (SMB, MM, Enterprise, Strategic) with best-in-class/good/normal/bleeding columns; NRR by segment and motion (consumption, PLG, enterprise, growth, acceptable, bleeding); monthly-to-annual churn conversion table (0.5% to 10% monthly with compounded annual); top-quartile vs median GRR and NRR by ACV band; five most common churn-math mistakes; voluntary vs involuntary mix by segment; Northwind SMB worked example (4.4% monthly = 41.6% annualized, NRR 99.4%); enterprise Northwind variant (4% annual GRR, 113% NRR); stage-and-scale targets by ARR; soft-churn trap (dormant accounts) benchmarks; reactivation/win-back benchmarks; 2021 vs 2026 valuation shift; full tools ecosystem map.',
  s8: 'Added Tomasz Tunguz adversarial counter — single-segment benchmark is misleading because it averages across motions (self-serve vs sales-assisted vs CSM-touched), with seven counter-cases: motion specificity inside a segment, category dynamics (discretionary vs load-bearing), valuation reality cuts back the other way (public market doesn\'t grant motion exemptions), benchmarks lag structural shifts (2026 vs 2024 data), when high churn is on purpose (tax prep, wedding planning, bootcamp), geographic/currency effects, and how the metric you choose shapes the org you build. Honest verdict: use segment benchmarks as first-pass, drill into motion/category/cohort/geo/intent for diagnosis.',
  s9: 'Cross-linked 29 related Pulse entries spanning the retention-adjacent topic space: the input metrics (ARR, NRR, CAC payback, Magic Number), customer success org design and playbooks, cohort analysis methodology, pricing and packaging for expansion, dunning and involuntary-churn recovery, Rule of 40 connection, PLG-vs-sales-led motion differences, onboarding programs for 30-day churn, QBR programs for enterprise renewal defense, LTV/CAC mechanics, renewal forecasting, consumption vs subscription pricing, land-and-expand motion design, AI impact on SaaS economics, customer health scoring, win-back motion, multi-year contract strategy and renewal cliff, segment-specific CS playbooks, pricing experiments, and customer concentration risk.',
  s10: 'SUBAGENT_VERIFIED. Deep rewrite of the churn-rate question for CFO/CRO/founder/board/RevOps audience covering SMB/MM/enterprise SaaS. (1) Churn taxonomy correct and exhaustive: GRR/NRR, logo/revenue, monthly/annualized, voluntary/involuntary, contraction. (2) Segment benchmarks anchored to multiple authoritative sources (SaaS Capital, OpenView, KeyBanc, ChartMogul, Bessemer) with correct bands by ACV. (3) Monthly-to-annual compounding math verified (5%/mo = 46.0% annualized via 1-(1-0.05)^12 = 0.4596). (4) NRR best-in-class examples (Snowflake 150%+, Datadog 130%+) verified against public disclosures. (5) BLS small business mortality (~20% first year, ~50% five-year) verified. (6) Simpson\'s paradox cohort example mechanically correct. (7) Two mermaid diagrams: computation decision tree (gross/net, logo/revenue, monthly/annual, voluntary/involuntary, cohort decomposition, benchmark read, driver diagnosis) and cohort revenue waterfall (start ARR -> voluntary -> involuntary -> contraction -> retained = GRR -> upsell -> cross-sell -> usage = NRR). (8) Counter-argument honestly represents Tunguz position (motion-specific benchmarks) without strawmanning. (9) Tools ecosystem comprehensive and current. (10) Zero banned phrases (leverage, utilize, delve, synergy, best-in-class used only in benchmark-band context where it\'s the term of art, world-class, cutting-edge, streamline, tapestry, today, ever-evolving, paradigm, game-changer). (11) Full structure: TL;DR + taxonomy + segment benchmarks + NRR axis + SMB structural drivers + enterprise structural drivers + cohort discipline + 2 mermaids + 40 sources + 12 quantified tables + 7-part Tunguz counter + 29 cross-links. (12) Word count target 9,200-9,800.'
};
...
console.log('PRE-FLIGHT q104 word counts: v5=' + wc(v5) + ' v6=' + wc(v6) + ' v7=' + wc(v7) + ' v8=' + wc(v8) + ' v9=' + wc(v9));
console.log('PRE-FLIGHT q104 char counts:  v5=' + v5.length + ' v6=' + v6.length + ' v7=' + v7.length + ' v8=' + v8.length + ' v9=' + v9.length);
if (wc(v9) > 10500) { console.error('ABORT: v9 word count ' + wc(v9) + ' exceeds 10500 cap'); process.exit(1); }
if (wc(v9) < 8500) { console.error('ABORT: v9 word count ' + wc(v9) + ' below 8500 floor'); process.exit(1); }

runPolish({ id: 'q104', tldr, core, flow, src, num, counter, links, sources, tags, notes }).catch(e => { console.error(e); process.exit(1); });
