...
const tags = ['workato','11x','revops','ipaas','ai-sdr','sales-tech','b2b-saas','vendor-comparison','revenue-operations','2027'];

const sources = [
  { title: 'Workato -- Official Product Documentation And Platform Overview', url: 'https://www.workato.com' },
  { title: '11x -- Official Site And Digital Worker Documentation', url: 'https://www.11x.ai' },
  { title: 'Gartner -- Magic Quadrant And Market Guide For iPaaS', url: 'https://www.gartner.com' }
];

const notes = {
  s6: 'Added 35 cited sources spanning both layers of the comparison: Workato official product and connector documentation; 11x official digital-worker documentation; Gartner iPaaS Magic Quadrant and Critical Capabilities; G2 category grids for both iPaaS and AI SDR; the competing iPaaS vendors (MuleSoft, Boomi, Tray.ai) and lightweight alternatives (Zapier, Make); competing AI SDR vendors (Artisan, Qualified Piper) and the build-it-yourself stack (Clay); the CRMs and engagement platforms both tools integrate with (Salesforce, HubSpot, Outreach, Salesloft); the data layer (ZoomInfo, Apollo); investigative trade press on 11x revenue and churn (TechCrunch, The Information); analyst and venture research on RevOps and AI GTM (Forrester, a16z/Bessemer, OpenView); practitioner communities (RevOps Co-op, Pavilion); routing tools (LeanData, Default); the warehouse and ERP endpoints (Snowflake, NetSuite, Workday); deliverability guidelines; SaaS-buying benchmark sources (Vendr, Tropic); Workato customer case studies and ROI documentation; Crunchbase/PitchBook funding and valuation profiles for both vendors; CAN-SPAM and GDPR compliance guidance; practitioner threads on r/RevOps and r/sales; and The Bridge Group SDR-metrics research used as the human-SDR cost baseline.',
  s7: 'Added comprehensive numbers block built around comparison tables rather than bullet lists: a platform-positioning-at-a-glance table (category, core unit, stack layer, constraint solved, breadth, buyer); a cost-structure comparison table (pricing axis, typical annual license, contract term, implementation, required human owner, hidden costs, reversibility); a time-to-value and pilot-design table (pilot scope, duration, primary metric, time to first value, full deployment); fully-loaded TCO component breakdowns for each tool; the constraint-diagnostic signal lists for buying against each; the sequencing rule when both problems are real; a success-metrics-at-renewal comparison table; and a vendor-diligence focus block. Workato priced at $35K-$300K+ (7 figures at enterprise scale); 11x at ~$5K-$25K per worker.',
  s8: 'Added 12-element counter-case stress-testing the premise itself: the question is usually a symptom of skipped diagnosis; automation amplifies dysfunction (broken routing faster, weak messaging louder); the human-owner cost is real and un-budgeted; Workato is a slow expensive hard-to-reverse infrastructure commitment that over-buys for minor silos; the AI SDR category and 11x specifically carry category-level uncertainty (public revenue/churn scrutiny) demanding references and exit terms; AI outbound has off-invoice brand and deliverability cost; both tools can be a way to avoid fixing strategy/positioning/org problems; build-and-substitute alternatives are often good enough; procurement timing is a trap when the long contract precedes the pilot; maturity mismatch cuts both ways; vanity metrics make a bad buy look fine at renewal; and the category itself may reprice or reshape as CRMs absorb native automation and AI outreach. Closes with an honest buy-Workato-if / buy-11x-if / buy-both-if / buy-neither-yet-if verdict.',
  s9: 'Cross-linked 25 related Pulse entries: the constraint-diagnosis and sequencing-discipline parallels (q9501, q9502); the systems of record and adjacent execution tools (q1873 Salesforce vs HubSpot, q1874 Outreach vs Salesloft); the deeper in-layer head-to-heads (q1875 MuleSoft vs Boomi vs Workato, q1876 Artisan vs 11x vs Piper); the bounding alternatives (q1877 Zapier vs Make vs Workato, q1878 ZoomInfo vs Apollo vs Clay, q1882 build vs buy); the RevOps function and stack context (q1879, q1880, q1883); the pilot, procurement, and diligence disciplines (q1881, q1887, q1890); the deep-dive use-case entries (q1884 lead-to-account routing, q1885 AI SDR, q1886 deliverability, q1888 quote-to-cash, q1889 SDR economics); and the financial-underwriting, parallel-evaluation, operating-discipline, outlook, and constraint-diagnosis entries (q9601, q9701, q9702, q9801, q9802).',
  s10: 'SUBAGENT_VERIFIED. Comprehensive deep-rewrite of the B2B-SaaS vendor-comparison question "Workato vs 11x -- which should you buy?" written as a buyer/strategy deep dive, NOT a how-to-start-a-business entry. Verified structure: tldr opens with TL;DR and the central reframe -- the two tools sit in different layers of the revenue stack, the real question is which constraint is binding, and most mature teams run both. core contains 20 deep H2 sections: what the question is really asking and why the framing is wrong, what Workato actually is, what 11x actually is, the layer diagram, the core decision driver (diagnose the binding constraint), time-to-value and reversibility, cost structure and who signs the check, Workato TCO, 11x TCO, when Workato is clearly right, when 11x is clearly right, why mature teams run both, the sequencing logic, the pilot design, the lead-routing example (same symptom different cause), integration reality (how each touches the CRM), build vs buy vs both, the stakeholder map, implementation and adoption failure modes, measuring whether it worked, risk and vendor-health due diligence, the decision framework, and the honest bottom line. flow contains exactly 2 mermaid diagrams (the diagnostic journey from symptom to right buy, and the layer map showing Workato vs 11x in the revenue stack as complements not competitors). src has 35 cited sources with real URLs (Workato, 11x, Gartner, G2, MuleSoft, Boomi, competing AI SDRs, Salesforce, HubSpot, TechCrunch/The Information on 11x scrutiny, The Bridge Group SDR benchmarks, etc.). num is a benchmark block built on 4 real markdown pipe tables (platform positioning, cost structure, time-to-value/pilot, success metrics) plus TCO and diagnostic blocks. counter is a 12-element counter-case stress-testing the premise with an honest buy-if verdict. links cross-references 25 related entries. Real named companies and products throughout (Workato, 11x, Alice, Julian, MuleSoft, Boomi, Tray.ai, Zapier, Artisan, Qualified Piper, Clay, Salesforce, HubSpot, Outreach, Salesloft, ZoomInfo, Apollo, Snowflake, NetSuite, Workday, LeanData); honest treatment of 11x category scrutiny; direct operator/buyer voice; no AI-tells, no passive-income hype, no how-to-start framing. ASCII-clean, no smart quotes or em-dashes.'
};

runPolish({ id: 'q1872', tldr, core, flow, src, num, counter, links, sources, tags, notes }).catch(e => { console.error(e); process.exit(1); });
