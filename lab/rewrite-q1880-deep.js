...
const tags = ['ai-forecasting','revenue-operations','ai-sdr-agents','sales-forecasting','revops-headcount','continuous-forecasting','clari-gong-einstein','forecast-calibration','revenue-strategy','2027'];

const sources = [
  { title: 'Clari -- Revenue Platform And Forecasting', url: 'https://www.clari.com' },
  { title: 'Gong -- Revenue Intelligence And Forecasting', url: 'https://www.gong.io' },
  { title: 'Gartner -- Sales Forecasting And Revenue Operations Research', url: 'https://www.gartner.com' }
];

const notes = {
  s6: 'Added 30 cited sources spanning the full landscape of this shift: the revenue-platform and AI-forecasting vendors (Clari and Clari Copilot, Gong and Gong Labs, Salesforce Sales Cloud forecasting and Agentforce, BoostUp, Aviso), the data-capture and CRM-hygiene layer (People.ai, Scratchpad), the sales-engagement platforms building native agent capability (Outreach, Salesloft, HubSpot), the AI-SDR agent platforms that trigger the whole shift (11x, Artisan, Qualified Piper, AiSDR), the analyst and benchmark sources (Gartner, Forrester, OpenView, The Bridge Group, Salesforce State of Sales), the practitioner communities (Pavilion, SaaStr, RevOps Co-op), investor analysis (a16z), and methodology references (MEDDIC/MEDDPICC, Winning By Design).',
  s7: 'Added a comprehensive numbers block built around real pipe tables: a manual-versus-continuous-AI forecast-accuracy comparison table (70-80% within 10% moving to 85-93% within 5-10%, with calibration, convergence, input, and dependency rows); the forecast-call tax per AE; a RevOps re-shape cost table for a representative 100-rep B2B SaaS org (5-7 person / $600K-$1.1M team becoming a 3-4 person / $400K-$700K team, tooling and total-cost lines, ~$150K-$400K visible savings); what shrinks 40-60% versus what grows; a table of the five things that replace manual forecasting and what each displaces; the manual-forecasting judgment failure modes and the new AI-forecasting failure modes; the contingencies the accuracy gain depends on; the seven-step transition sequence; and what does not get replaced.',
  s8: 'Added a 9-element counter-case stress-testing the thesis: the AI-SDR trigger may be messier and slower (augmentation not replacement) than assumed; garbage-in is still the dominant reality and AI forecasting on a poorly-instrumented funnel is worse than manual; the self-fulfilling feedback loop is structural not a corner case; forecasting is a commitment act and commitment is irreducibly human; the accuracy ceiling in volatile/new/low-sample segments is real and low; the headcount savings are overstated and the wrong reason to do it; institutional skill atrophy is a slow invisible expensive failure; vendor incentives pollute every accuracy claim; and the transition cost and disruption are high with half-finished states worse than either pure state -- closing with an honest verdict that the direction holds but the thesis is routinely oversold on four specific points.',
  s9: 'Cross-linked 25 related Pulse entries: the directly-referenced RevOps headcount companion (q1898), the cluster of second-order effects on roles and comp and quota (q1881, q1882, q1883, q1899, q1900), the AI-SDR agent buyer context (q1884), the revenue-intelligence and tooling background (q1885, q9701), the transition-mechanics entries (q1886 instrument, q1887 calibration, q1888 exception review, q1890 governance, q1895 re-skill, q1897 transition off the call), the counter-argument deep dives (q1891 feedback loop, q1892 honest accuracy), the cross-functional and methodology entries (q1893 finance, q1894 MEDDIC, q1896 conversation intelligence, q1889 surviving roles), the broader-stack context (q9601, q9801), and the two benchmark reference entries (q9501, q9502).',
  s10: 'SUBAGENT_VERIFIED. Comprehensive deep-rewrite of the RevOps/sales-strategy question "What replaces manual forecasting if AI agents replace SDRs natively?" -- written as a strategy deep dive, not a how-to-start-a-business entry. Verified structure: tldr opens with TL;DR and the core thesis (manual forecasting is structurally replaced, not improved, because it existed as a compensating control for low-resolution SDR-generated pipeline data; the replacement is continuous signal-grounded calibrated forecasting plus human judgment moved up the stack); core contains 15 deep H2 sections covering what manual forecasting actually is and why it existed, why the SDR-to-AI shift is the specific trigger, what actually replaces it (continuous signal-grounded probabilistic forecasting), the real tool landscape (Clari, Gong, Salesforce/Einstein/Agentforce, BoostUp, Aviso, People.ai, Scratchpad, plus the 11x/Artisan/Qualified/AiSDR AI-SDR layer), the forecast call collapsing into an exception review, the RevOps re-shape (what shrinks 40-60%, what grows), the unit of forecasting changing from the rep story to the signal, human judgment moving up the stack, the honest accuracy question (70-80% to 85-93%), the concrete cost and headcount math, what does NOT get replaced, the failure modes, the transition sequence, the second-order effects, and an honest synthesis. flow contains exactly 2 mermaid diagrams (the replacement architecture from manual roll-up to governed continuous forecast, and the RevOps-leader transition decision path). src has 30 cited sources with real URLs; num is a benchmark block with 5+ real markdown pipe tables; counter is a 9-element counter-case with an honest verdict; links cross-references 25 related entries. Real named companies and platforms throughout, realistic 2027 RevOps economics, direct operator voice, no passive-income or how-to-start hype, ASCII-clean with no smart quotes or em-dashes.'
};

runPolish({ id: 'q1880', tldr, core, flow, src, num, counter, links, sources, tags, notes }).catch(e => { console.error(e); process.exit(1); });
