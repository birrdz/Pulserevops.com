...
The reason this signal carries so much weight is that a pricing change is one of the few events that stress-tests the burn multiple from *both* sides at once. On the denominator side, a reprice can suppress net new ARR temporarily — confused buyers stall, the sales team needs a quarter to internalize the new motion, and existing customers up for renewal may push back on a higher number. On the numerator side, a packaging change usually carries real cost: sales enablement, billing-system rework, customer-success outreach, and sometimes grandfathering concessions that show up as discounts. A naive expectation is that the multiple should *worsen* during the transition quarter and only improve later. So when the multiple holds flat or improves *through* the change itself, the company has demonstrated something genuinely hard: it raised the value it captures per customer faster than the transition cost it incurred. That is a structural win, not a timing artifact.

There is an important discipline in how you read this signal, because a pricing change can also produce a *false* celebrate. If you reprice and the multiple drops sharply in the very next quarter, ask whether the improvement is durable or a one-time pull-forward — a wave of existing customers signing early renewals at the new rate inflates net new ARR in the transition quarter and deflates it in the following two. The honest test is the one used throughout this entry: read the trailing-twelve-month multiple across the transition, not the single quarter that contains the reprice. If TTM is flat or down across the full year spanning the change, the move was genuinely accretive; if only the transition quarter looks good and the quarters after it sag, you pulled revenue forward rather than created it.

| Pricing-change outcome | Transition-quarter multiple | Two quarters later | Verdict |
|---|---|---|---|
| Genuinely accretive reprice | Flat or slightly better | Holds or improves further | Celebrate — durable value capture |
| Pull-forward of renewals | Sharply better | Sags noticeably | False signal — timing artifact |
| Disruptive reprice | Worse (churn spike) | Recovers slowly or not at all | Worry — change was mishandled |
| Mixed / partial migration | Noisy, hard to read | Depends on migration pace | Investigate cohort by cohort |

A practical example: a company moving from a flat per-seat plan to a usage-based model should expect net new ARR to be *lumpier* afterward, not necessarily lower. The right way to celebrate a clean usage-based transition is to show the board the TTM burn multiple holding steady while the *committed* portion of recurring revenue — the floor under the usage model — keeps growing. That separates a structurally sound transition from a quarter that merely caught a consumption spike.
...
The mechanics of a cohort burn-efficiency view are worth spelling out, because most finance teams have never built one. The blended burn multiple divides *all* burn in a period by *all* net new ARR in that period — a whole-company question. A cohort view asks a narrower, more revealing one: for the customers acquired in a given quarter, how much burn did it take to land and grow them, and how is that figure trending across successive acquisition quarters? The payoff is that a cohort view can detect an improving engine *before* the blended number moves, because a single strong recent cohort is diluted by older cohorts in the blended figure but stands out clearly in the cohort series.

| Acquisition cohort | Cohort burn to land | Cohort net new ARR (incl. expansion) | Cohort burn multiple | Trend |
|---|---|---|---|---|
| Cohort 1 (oldest) | $3.4M | $1.9M | 1.79x | Baseline |
| Cohort 2 | $3.5M | $2.2M | 1.59x | Improving |
| Cohort 3 | $3.3M | $2.4M | 1.38x | Improving |
| Cohort 4 (newest) | $3.2M | $2.6M | 1.23x | Improving |

A board that sees a table like this — four consecutive cohorts each more efficient than the last — has far more reason to trust the engine than a board shown one good blended quarter. The cohort view is also self-correcting against the deal-timing noise described in section 3: a lucky quarter where three big logos closed late inflates the *blended* multiple for that period but is correctly attributed to the cohort it belongs to, so it does not contaminate the trend. When you can show a clean, monotonically improving cohort series, that is the most credible celebrate signal in this entire entry, because it is the one that is hardest to fake and hardest to attribute to luck.
