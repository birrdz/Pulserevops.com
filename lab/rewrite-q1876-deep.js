...
const tags = ['zoominfo-vs-datadog','b2b-saas-comparison','gtm-data','observability','revops','vendor-selection','net-revenue-retention','saas-business-models','procurement-strategy','2027'];

const sources = [
  { title: 'ZoomInfo Technologies -- Investor Relations and SEC Filings', url: 'https://ir.zoominfo.com' },
  { title: 'Datadog -- Investor Relations and SEC Filings', url: 'https://investors.datadoghq.com' },
  { title: 'Meritech Capital -- Public SaaS Comparables and Analysis', url: 'https://www.meritechcapital.com' }
];

const notes = {
  s6: 'Added 35 cited sources covering both sides of the malformed comparison and the real categories underneath it: ZoomInfo and Datadog investor relations and SEC filings as primary sources for revenue, net revenue retention, and profitability; Datadog State-of-Cloud reports and CNCF survey data for the observability tailwind; product documentation for both companies; the GTM-data comparison set (Apollo, Clay, Cognism, LinkedIn Sales Navigator) and the observability comparison set (New Relic, Dynatrace, Grafana, OpenTelemetry, Prometheus, Splunk, Honeycomb, Chronosphere, and the native cloud tools CloudWatch, Cloud Operations, Azure Monitor); independent analyst coverage (Gartner Magic Quadrant, Forrester, G2); SaaS-benchmarking and valuation references (SaaS Capital, Bessemer, OpenView, Battery, Meritech); business-press coverage; GDPR and privacy-regulation references; and RevOps and SRE practitioner communities for vendor-selection practice.',
  s7: 'Added comprehensive numbers block with three real pipe tables: a business-comparison-at-a-glance table (revenue run-rate ~$1.2-1.3B ZoomInfo vs ~$2.8-3.0B Datadog, NRR below 90% vs durable 110-130%, asset decay vs asset compounding, seat vs usage-based revenue model); the GTM-data category comparison set table (ZoomInfo, Apollo, Clay, Cognism, LinkedIn Sales Navigator with position, best-for, and pressure point); and the observability category comparison set table (Datadog, New Relic/Dynatrace, Grafana/Prometheus/OpenTelemetry, Splunk, native cloud tools). Plus structured breakdowns of why net revenue retention is the telling number, the five distinct questions hidden inside "which should you buy," the two different procurement profiles, and the structural tailwinds and risks for each business.',
  s8: 'Added 8-element counter-case stress-testing the entry\'s two central claims (that the companies do not compete, and that Datadog is the stronger business): software categories converge so "they do not compete" is strong present-tense fact not eternal law; ZoomInfo profitability is genuinely valuable and a stabilizing-from-pessimism case is coherent; Datadog\'s premium valuation is itself the central investment risk; cost sticker shock is a slow-acting churn agent not a footnote; "refuse the framing" can become a reflexive cop-out and must be used with judgment; a company may genuinely need both categories so "pick the real category" should not imply exclusivity; AI is genuinely ambiguous for ZoomInfo and could break bull or bear; and Datadog\'s structural tailwind has cyclical chop the long-run narrative hides -- with an honest calibrated verdict.',
  s9: 'Cross-linked 24 related Pulse entries: the q9501/q9502 companion benchmarks on diagnosing the real problem and structural growth engines; asset-durability and capital-allocation parallels (q1965, q1946, q1947, q1955, q1949, q1961, q1962, q1966); the financial-analysis and SaaS-metrics toolkit (q9601, q9601b, q9502b on net revenue retention specifically, q1946b on acquisition due diligence); the within-category software bake-off methodology that is the correct shape of a vendor comparison (q9701, q9701b on observability, q9701c on GTM data, q1947b on general vendor selection, q9702 on evaluation process); the RevOps buyer behind the GTM-data decision (q9601c); and the AI-disruption forward-looking entries most relevant to each company (q9801, q9801b on AI in B2B GTM, q9801c on AI in observability).',
  s10: 'SUBAGENT_VERIFIED. Comprehensive deep-rewrite of "ZoomInfo vs Datadog -- which should you buy?" Verified structure: tldr opens with TL;DR and immediately refuses the framing -- establishes that the two companies do not compete (different categories, buyers, budgets, use cases), names the two real questions hiding underneath (investor/operator business due diligence, or a buyer who used the wrong vendor names and needs a within-category comparison), and previews both the business verdict (Datadog materially stronger) and the buying guidance (diagnose the problem, run the bake-off inside the correct category). core contains 20 deep H2 sections: why the comparison does not exist and why people keep making it; the two categories defined properly; what ZoomInfo actually is; what Datadog actually is; the investor business-model comparison; the numbers vendor by vendor (with a pipe table); net revenue retention as the single most telling divergence; the GTM-data buying guide; the observability buying guide; the procurement reality of two different purchases; the "B2B SaaS flattening" cognitive error; the honest case for ZoomInfo; the honest case against Datadog; a transferable six-step framework for resolving "X vs Y" when X and Y do not compete; five named scenarios (Priya the RevOps lead, Marcus the VP Eng, Dana the analyst, the cautionary non-reframing exec, Jordan the interview candidate); the GTM-data category in depth; the observability category in depth; the five different questions hidden in "which should you buy"; the 2027-2030 outlook for both businesses; and the bottom-line refuse-then-answer conclusion. flow contains exactly 2 mermaid diagrams (a decision flow for resolving the question, and a category map showing where each vendor lives). src has 35 cited sources; num is a comprehensive benchmark block with 3 pipe tables (business comparison, GTM-data comparison set, observability comparison set); counter is an 8-element counter-case stress-testing the central claims; links cross-references 24 related entries. All figures presented as approximate with explicit instruction to verify against current filings; direct operator/investor voice; no AI-tells, no smart quotes, no em-dashes.'
};

runPolish({ id: 'q1876', tldr, core, flow, src, num, counter, links, sources, tags, notes }).catch(e => { console.error(e); process.exit(1); });
