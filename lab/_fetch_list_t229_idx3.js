...
https.get('https://pulserevops.com/.netlify/functions/pulse-machine-library-list?recent=5000', res => {
  ...
  res.on('end', () => {
    ...
    fs.writeFileSync('C:/Users/koryj/website/lab/_list_t229_idx3.json', JSON.stringify(j));
    ...
    elig.slice(0, 12).forEach((e, i) => console.log('  idx' + i + ' ' + e.id + ' ts=' + e.ts + ' qs=' + e.quality_score + ' fv=' + e.format_v + ' :: ' + e.question));
    const target = elig[3];
    console.log('TARGET idx3 = ' + (target ? target.id + ' :: ' + target.question : 'NONE'));
  });
