...
https.get('https://pulserevops.com/.netlify/functions/pulse-machine-library-list?recent=5000', res => {
  ...
  res.on('end', () => {
    ...
    fs.writeFileSync('C:/Users/koryj/website/lab/_list_t229_idx14.json', JSON.stringify(j));
    ...
    elig.slice(0, 20).forEach((e, i) => console.log('  idx' + i + ' ' + e.id + ' ts=' + e.ts + ' qs=' + e.quality_score + ' fv=' + e.format_v + ' :: ' + e.question));
    const target = elig[14];
    console.log('TARGET idx14 = ' + (target ? target.id + ' :: ' + target.question : 'NONE'));
  });
