...
const entries = Array.isArray(j) ? j : (j.entries || j.items || []);
...
elig.slice(0, 25).forEach((e, i) => console.log(i, e.id, 'ts=' + e.ts, 'qs=' + e.quality_score, 'fv=' + e.format_v, '|', (e.question || '').slice(0, 75)));
console.log('---TARGET (index 18)---');
const t = elig[18];
console.log(JSON.stringify({ id: t.id, question: t.question, tags: t.tags, ts: t.ts }, null, 2));
fs.writeFileSync('C:/Users/koryj/website/lab/_target_t229.json', JSON.stringify(t));
fs.writeFileSync('C:/Users/koryj/website/lab/_alldids_t229.json', JSON.stringify(entries.map(e => e.id)));
