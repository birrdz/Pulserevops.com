// v15 ONE-RUNG MODE t189 — q22 rung 5→6
// Word floor: 5,500w. No format_v stamp (only Q+1=10 gets stamped).

const fs = require('fs');
const https = require('https');

const new_answer = `# CRO Track Record Red Flags: A Complete Diligence Playbook
...
console.log('Word count:', new_answer.split(/\s+/).filter(Boolean).length);
...
fs.writeFileSync('lab/_q22_v15_t189_body.json', body);
...
const req = https.request({
  ...
  res.on('end', () => {
    ...
    fs.writeFileSync('lab/_q22_v15_t189_response.json', data);
  });
});
req.on('error', e => { console.log('ERR:', e.message); fs.writeFileSync('lab/_q22_v15_t189_error.txt', e.message); });
req.on('timeout', () => { console.log('TIMEOUT'); req.destroy(); fs.writeFileSync('lab/_q22_v15_t189_error.txt','TIMEOUT'); });
req.write(body);
req.end();
