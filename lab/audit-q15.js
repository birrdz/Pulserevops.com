// Quick audit of q15 against the 6 gold format elements.
...
(async () => {
  ...
  const e = await store.get('answers/q15.json', { type: 'json' });
  if (!e) { console.error('q15 not found'); process.exit(1); }
  const ans = e.answer || '';
  fs.writeFileSync(path.join(__dirname, 'q15_raw.md'), ans, 'utf8');
  ...
  const audit = {
    ...
    elements: {
      ...
      e5_real_companies: {
        ...
        spiff: /Spiff/i.test(ans),
        captivateiq: /CaptivateIQ/i.test(ans),
        performio: /Performio/i.test(ans),
        xactly: /Xactly/i.test(ans),
        quotapath: /QuotaPath/i.test(ans),
