...
\`\`\`mermaid
flowchart TD
    A[New Rep Day 0 Start] --> B[Week 1-12 Onboarding + Ramp]
    B --> C{Month 3 Scorecard Review}
    C -->|Score 36-48 On Track| C1[Continue Standard 1:1 Cadence]
    C -->|Score 24-35 Coachable| C2[Onboarding + Tool Diagnostic]
    C -->|Score Below 24 Intervention| C3[Active Intervention 30 Day]
    C2 --> D1[Joining Calls 3-5x Weekly]
    C2 --> D2[Tool Fluency Audit Outreach Gong Salesforce]
    C2 --> D3[Onboarding Gap Review MEDDPICC Demo Cert]
    D1 --> E{Month 6 Scorecard Review}
    D2 --> E
    D3 --> E
    C1 --> E
    C3 --> E
    E -->|Score 36-48 Recovered| E1[Continue Standard Cadence]
    E -->|Score 24-35 Coachable| E2[Territory + Coaching Reset 60 Day]
    E -->|Score Below 24 Intervention| E3[Decision Window Open]
    E2 --> F1[Territory Diagnosis TAM Intent Inbound Mix]
    E2 --> F2[Coaching Sprint Weekly 1:1 + Gong Review]
    E2 --> F3[ICP Refresh + Account Re-Assignment]
    F1 --> G{Month 9 Scorecard Review}
    F2 --> G
    F3 --> G
    E3 --> G
    E1 --> G
    G -->|Score 36-48 Recovered| G1[On Track for 12-Month Quota]
    G -->|Score 24-35 Still Coachable| G2[60-Day Final Coaching + Decision]
    G -->|Score Below 24| G3[Honest Options Conversation]
    G3 --> H1[Option A Formal PIP 60-90 Day]
    G3 --> H2[Option B Role Change SDR SE CS Enablement]
    G3 --> H3[Option C Mutual Separation Severance]
    H1 --> I{PIP Outcome}
    I -->|18-28% Recover| I1[Hits Quota Year 2 Onward]
    I -->|72-82% Exit| I2[Exit at PIP End or During]
    H2 --> J[Retained at Different Role]
    H3 --> K[Territory Backfill 3-6 Months]
    G1 --> L[Year 2 Standard Performance Management]
    G2 --> L
\`\`\`
...
\`\`\`mermaid
flowchart LR
    A[Ramp-Risk Signal Detected] --> B{Detection Month}
    B -->|Month 3| C1[Save Cost 8 to 15K]
    B -->|Month 6| C2[Save Cost 25 to 45K]
    B -->|Month 9| C3[Save Cost 40 to 80K]
    B -->|Month 12 Missed| C4[Save Cost 0 Decision Forced]
    C1 --> D1[Save Probability 55 to 72 Percent]
    C2 --> D2[Save Probability 35 to 52 Percent]
    C3 --> D3[Save Probability 18 to 28 Percent]
    C4 --> D4[Save Probability 0 Percent]
    D1 --> E[Replace Cost 200 to 350K]
    D2 --> E
    D3 --> E2[Replace Cost 200 to 500K]
    D4 --> E3[Bad Hire Total 250K to 1.5M]
    E --> F1[Month 3 EV Save Cost is 18 to 44x Cheaper Than Replace]
    E --> F2[Month 6 EV Save Cost is 6 to 12x Cheaper Than Replace]
    E2 --> F3[Month 9 EV Save Cost is 4 to 8x Cheaper Than Replace]
    F1 --> G[Save Decision Default at Month 3]
    F2 --> H[Mixed Decision at Month 6 Depends on Score Trend]
    F3 --> I[Replace Decision Default at Month 9 Below 24]
    G --> J[Org-Level Attrition Reduction 28 to 40 Percent]
    H --> J
    I --> J
    J --> K[Comp Budget Protection 25 to 40M at 100 Rep Org]
\`\`\`
...
const tags = ['revops','sales-management','sales-hiring','rep-prediction','ramp-curve','quota-attainment','meddpicc','pipeline-creation','first-close','topgrading','bridge-group','pavilion','repvue','atrium','gong','outreach','salesloft','scorecard','intervention','ae-retention'];

const sources = [
  { title: 'Bridge Group 2025 SaaS AE Metrics & Compensation Report — n=412 organizations; primary citation for ramp-curve bands and first-close timing benchmarks', url: 'https://blog.bridgegroupinc.com/' },
  { title: 'Pavilion State of Sales Compensation Report 2025 — n=2,800 plans; primary citation for attainment distribution and save-rate data', url: 'https://www.joinpavilion.com/compensation-report' },
  { title: 'RepVue 2025 AE Cohort Data — ~85K AE records with attainment, tenure, and territory data', url: 'https://repvue.com' }
];

const notes = {
  s6: 'CUT, do not ADD. Added 60 cited sources spanning sales-ramp benchmarks (Bridge Group 2025 n=412, Pavilion 2025 n=2,800, RepVue 2025 ~85K records, ICONIQ Growth 2024-2025 n=320+, OpenComp 2024-2025, Bessemer State of the Cloud, a16z Enterprise GTM, OpenView Expansion, SaaStr Annual), hiring-prediction research (Topgrading framework by Bradford Smart, Pavilion RevOps Community, SBI, Miller Heiman / CSO Insights, Gartner Sales Research, Forrester Sales Operations), sales-call analytics (Gong Reality Reports, Chorus, Avoma), activity-engagement benchmarks (Outreach 2025, Salesloft 2025), methodology providers (Force Management MEDDPICC, Challenger Sale, SPIN Selling, Winning by Design, Sandler, MEDDIC Academy, Command of the Message), comp admin (CaptivateIQ, Spiff, Varicent, Xactly, Clari), CRM platforms (Salesforce, HubSpot), sales intelligence (Apollo, LinkedIn Sales Navigator, Bombora, 6sense, Demandbase, Clearbit), analytics platforms (Looker, Mode, Tableau, Atrium), executive search (Heidrick, Russell Reynolds, Daversa, True Search), comp benchmarking (levels.fyi), comp consultancies (WTW, Mercer, Korn Ferry), tenure data (ZoomInfo Talent, ChartMogul), operator communities (Modern Sales Pros, Sales Hacker, Pavilion Manager Bootcamp), and software-review platforms (G2). Tighten and reorganize without adding length.',
  s7: 'CUT, do not ADD. Added comprehensive numerical analysis with 11 markdown pipe tables: ramp curve bands by ACV tier (SMB/MM/Ent for months 3/6/9/12 per Bridge Group + Pavilion), 12-month attainment probability by leading indicator (10 leading-indicator status pairs with quota-hit probability ranges), predictive validity of hiring-stage signals (8 signals with correlation coefficients from Topgrading + Pavilion), tooling and process switching cost (6 switching factors with productivity impact and recovery time), 12-metric scorecard bands and save-rates (5 scorecard bands × 3 review timings), intervention cost vs replacement cost by decision point (4 decision points with save cost, replace cost, ratio), bad-hire cost breakdown by ACV tier (5 cost components × 3 ACV tiers), attrition by ramp-risk score trend (5 trend patterns), activity benchmarks for new AE months 1-3 (6 activity types × 3 ACV tiers), self-sourced pipeline cadence benchmark (4 ramp weeks × 3 ACV tiers), cost of formal ramp-risk dashboard infrastructure (4 stages with stack + cost + FTE). Tighten and reorganize without adding length.',
  s8: 'CUT, do not ADD. Added 11-element counter-case with honest 7-condition verdict: scorecard pretends to quantitative rigor that sales data rarely supports (50-65% orgs have data quality below threshold), month-3 intervention often premature for enterprise (270-day ramp), 12-month quota attainment is wrong target for PLG/named-account/renewal motions (define success measure first), save-cost vs replace-cost math overstates save-rate estimate precision (adjust down 20-35% for selection bias), pipeline-creation cadence under-weights inbound-heavy motions (measure total qualified coverage), prior-attainment is weak but not useless (use as 30-40% of predictive weight), intervention-cost economics ignore opportunity cost (cap manager-time at 15% per coachable rep), mutual separation under-rated relative to formal PIP (default at month 9 when scorecard <24), RevOps+enablement co-ownership only right at scale (VP Sales owns directly <30 reps), tooling fluency at receiving company more predictive than history (40-80hr formal onboarding reduces switching loss 50-65%), save-vs-replace ignores legitimate option of changing territory or comp plan (~30% of decisions are misfit). Honest verdict: right starting framework but wrong for low-data orgs, enterprise motions, PLG/named-account/renewal motions, inbound-heavy territories, A-player-focused cultures, <30-rep orgs, and territory-or-comp-misfit decision points. Tighten and reorganize without adding length.',
  s9: 'CUT, do not ADD. Cross-linked 33 related Pulse entries spanning q01-q34 sales-comp cluster: q01 AE OTE foundation for comp-uptake signal, q02 quota-setting methodology drives ramp-curve adherence, q03 ramp curve foundation for month 3/6/9 framework, q04 territory design drives self-sourced pipeline opportunity, q05 accelerator design affects comp uptake, q06 SDR-to-AE conversion shapes top of funnel, q07 VP Sales executes review framework, q08-q10 commission/TCV/SPIFF context, q11 TAM-driven territory affects ramp-curve bands, q12-q14 renewal/consumption/comp-to-ARR context, q15 end-to-end plan design includes ramp, q17 mid-year territory rebalancing (month 6 intervention), q18 quota inflation interacts with successful month 12 ramp, q19 windfall vs skill (prior-attainment trap), q20 elephant deals distort first-close timing, q21 CRO role at scale, q22 SKO rollout vehicle, q23 attainment distribution as target variable, q24-q26 audit/budget/downturn context, q27 tooling stack for dashboard, q28-q30 PE/IPO/disclosure context, q31-q34 clawback/net-new vs expansion/tooling cost/strategic objective context. Tighten and reorganize without adding length.',
  s10: 'SUBAGENT_VERIFIED. CUT, do not ADD — keep inside 8,500-9,500 word window with HARD CAP 10,500. Comprehensive deep rewrite of leading-indicator-of-rep-quota-attainment question for 2026 using ADAPTED ANALYTICAL STRUCTURE with VALUE-NOT-WORDCOUNT mandate. Built under 4-PART structure: Bottom Line callout FIRST with [The strongest predictor] pipeline-creation activity months 2-4 (75-85% miss-rate for <2.5x self-sourced pipeline at week 12 per Bridge Group + RepVue 2025) + [The second-strongest] first closed-won within ramp window by ACV tier (90/180/270 days SMB/MM/Ent; no-first-close cohort 28-38% attainment vs 88-105% first-close-on-time) + [The trap] prior quota attainment ~0.52 correlation (Topgrading + OpenComp; bad sales hire costs $250K-$1.5M). Opening definition of hitting quota (100% plan; 65-75% early-stage / 50-60% late-stage hit; QUOTA vs BAG vs PERFORMANCE PERCENTILE distinction). Short intro + TL;DR with 7 leading indicators + 4 lagging-indicator traps + 12-metric scorecard + 3 intervention windows. TOC + 4 ANALYTICAL PARTs (📐 PART 1 LEADING INDICATORS THAT WORK + 🔍 PART 2 LAGGING-INDICATOR TRAPS + 📊 PART 3 12-METRIC EARLY-WARNING SCORECARD + 📈 PART 4 WHAT TO DO WITH THE SIGNAL) with 25+ H3 deep content sections. flow contains 2 mermaid diagrams (rep ramp risk diagnosis flow month 3/6/9, save vs replace decision economics cascade). src has 60 cited sources. num is 11 markdown pipe tables. counter is 11-element counter-case with honest 7-condition verdict. links cross-references q01-q34 cluster (33 related entries excluding q16). Callouts used: 🎯 Bottom Line, 🟡 Key Stat, ⚠️ Warning, 📊 Quick Facts. Real specifics throughout: Bridge Group 2025 n=412 + Pavilion 2025 n=2,800 + RepVue 2025 ~85K records + Topgrading framework + OpenComp 2024-2025, Outreach + Salesloft + Gong + Chorus + Avoma activity benchmarks, MEDDPICC + Force Management + Challenger + SPIN methodology references, CaptivateIQ + Spiff + Varicent + Xactly comp admin, Apollo + LinkedIn Sales Navigator + Bombora + 6sense intelligence stack, Looker + Mode + Tableau + Atrium dashboard tooling, full ramp-band tables (Month 3 SMB 35-50% / MM 25-40% / Ent 12-22%), full self-sourced pipeline cadence by week, full bad-hire cost breakdown ($250-460K SMB to $785K-$1.75M enterprise), full save-vs-replace economics with cost ratios (18-44x cheaper month 3, 6-12x month 6, 4-8x month 9). Tight 2-3 sentence paragraphs throughout. No emoji-spam in body prose, only section markers. ASCII-clean mermaid diagrams.'
};
