// Quick audit of q12 against the 6 gold format elements.
// q12 -- "What's the typical CRO base salary in NYC vs SF vs remote in [year]?"
...
(async () => {
  ...
  const e = await store.get('answers/q12.json', { type: 'json' });
  if (!e) { console.error('q12 not found'); process.exit(1); }
  const ans = e.answer || '';
  fs.writeFileSync(path.join(__dirname, 'q12_raw.md'), ans, 'utf8');
  ...
  const audit = {
    ...
    elements: {
      ...
      e5_real_companies: {
        ...
        bessemer: /Bessemer|BVP/i.test(ans),
        worldatwork: /WorldatWork/i.test(ans),
        alexander_group: /Alexander Group/i.test(ans),
        heidrick: /Heidrick/i.test(ans),
        zs_associates: /ZS Associates/i.test(ans),
        korn_ferry: /Korn Ferry/i.test(ans),
        equilar: /Equilar/i.test(ans),
        carta: /Carta/i.test(ans),
        compensia: /Compensia/i.test(ans),
        mercer: /Mercer/i.test(ans),
        radford: /Radford|Aon/i.test(ans),
        fw_cook: /FW Cook|F\.W\. Cook/i.test(ans),
        pearl_meyer: /Pearl Meyer/i.test(ans),
        hubspot: /HubSpot|HUBS/i.test(ans),
        yamini_rangan: /Yamini Rangan/i.test(ans),
        snowflake: /Snowflake|SNOW/i.test(ans),
        mongodb: /MongoDB|MDB/i.test(ans),
