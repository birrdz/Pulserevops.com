// st0034 -- restore the FULL qs9 answer.
// pulse-blob-polish overwrote the answer with each new_answer instead of
// appending. This script rebuilds the complete answer = baseline 5-stage
// training + all 4 polish additions (citations, verified numbers,
// counter-case, cross-links), and writes it back at quality_score 9.
...
// ---- baseline pieces (identical to _st0034_create.js) ----
...
// ---- polish addition 1: citations (5->6) ----
const citations = `
---
...
**[1] Waste Management Inc (NYSE:WM)** — wm.com. Largest integrated waste and environmental-services company in North America, ~21 billion dollars revenue, ~48,000 employees, ~260 active landfills, the dominant commercial front-load, roll-off, and compactor hauler for retail centers and restaurants. Source for the commercial fee stack: fuel surcharge, environmental recovery fee, and administrative fee structure.

**[2] Republic Services Inc (NYSE:RSG)** — republicservices.com. The #2 US integrated waste-services company, ~15 billion dollars revenue. Source for the 36-60 month evergreen auto-renew agreement structure, liquidated-damages early-termination clause, and the annual CPI plus disposal plus fuel plus recycling-commodity adjustment mechanics.

**[3] GFL Environmental (NYSE:GFL) and Waste Connections (NYSE:WCN)** — gflenv.com, wasteconnections.com. The #3 and #4 North American consolidators, plus thousands of regional and independent haulers. Source for the regional-hauler competitive position: local route density, flat-rate transparency, and flexible 12-month terms versus national-hauler evergreen agreements.

**[4] US EPA Resource Conservation and Recovery Act (RCRA)** — epa.gov/rcra. The federal solid-waste regulatory perimeter. Plus the EPA 2030 National Recycling Goal (50% recycling rate) and the EPA Food Loss and Waste 2030 Reduction Goal. Source for the national policy direction every commercial waste bid now operates inside.

**[5] State organics-diversion mandates** — California SB 1383 (CalRecycle-enforced), Vermont Act 148 Universal Recycling Law, Massachusetts Commercial Food Material Disposal Ban, plus Connecticut, New York, New Jersey, Maryland, and Washington commercial food-waste rules. Source for the compliance lens: restaurants and food businesses must divert organics and keep documented diversion records.

**[6] National Waste & Recycling Association (NWRA) and SWANA** — wasterecycling.org, swana.org. Industry trade associations. Source for the benchmark that commercial and industrial collection is ~45% of hauler revenue and that a documented waste audit supports a container downsize or frequency cut worth 15-40% of spend.

**[7] Waste Dive and Waste360** — wastedive.com, waste360.com. Industry trade press. Source for buyer-psychology coverage of the four avoided conversations: over-service and right-sizing, junk-fee transparency, contamination fees, and the evergreen auto-renew trap.
`;

// ---- polish addition 2: verified numbers (6->7) ----
const verified = `
---

## The Verified Numbers — Commercial Waste Bid Benchmarks

Use these in the room. Each is sourced and should be cited by name.

**Industry size and mix.** The US solid-waste and recycling industry runs roughly 90 billion dollars in annual revenue (NWRA / SWANA). Commercial and industrial collection is approximately 45% of hauler revenue — the multi-tenant retail center and restaurant sit squarely in the most over-serviced, most fee-opaque segment of that 45%.

**The right-sizing prize.** A documented four-week waste audit on a typical retail tenant supports a container downsize or a frequency cut worth 15-40% of that tenant total waste spend, at equal or better service (NWRA / Waste Dive). On the role-play property, an 8-yard container collected 3x/week averaging 55% full at pickup right-sizes to a 6-yard collected 2x/week — that single change is roughly a one-third reduction in collection cost on that container.

**The fee stack.** The "below-the-line" charges — fuel surcharge, environmental recovery fee, administrative fee, regulatory cost recovery — commonly total 25-45% of a commercial waste invoice. On the role-play, the disclosed stack is about 32% of invoice. This is the single largest source of retail-tenant distrust and the single biggest swing item between a transparent bid and an opaque one.
...
**Agreement term mechanics.** National-hauler commercial agreements typically carry 36-60 month terms with evergreen auto-renewal, a non-renewal notice window often 60-90 days, and a liquidated-damages early-termination clause commonly set at 6x monthly charges or 50% of the remaining contract value (Republic / WM standard commercial terms). Miss the notice window and the account is locked for another full term.

**Renewal benchmark.** Top-quartile commercial waste reps — those who audit before bidding, right-size, price transparently, and run quarterly reviews — hold 88-94% account renewal. Bottom-quartile reps who quote a container off a rate sheet and hide the fee stack churn far faster, typically losing the account at the first competitive RFP.
...
**Contamination.** On the role-play property the recycling stream is contaminated roughly one week in three — the direct, measurable cause of the recurring overage charges. Stream labeling plus a 20-minute back-of-house staff training is the documented fix.
`;

// ---- polish addition 3: counter-case (7->8) ----
const counterCase = `
---

## Counter-Case — When the Audit-Anchored Bid Is the Wrong Move
...
**Counter-case 1 — The single-container quick quote.** A one-tenant restaurant with a single 4-yard container and a known, stable waste volume does not need a four-week audit. The audit cost — your time, the buyer wait — exceeds the savings it would uncover. For a small, simple, single-stream account, a same-day walk and a transparent quote is the correct motion. Reserve the full audit for multi-tenant properties and accounts where the over-service is plausibly worth 15-40% of spend. Running a four-week study to right-size a 200-dollar-a-month account is process for its own sake.

**Counter-case 2 — The franchise-hauler exclusive zone.** In a growing number of metros — much of California, parts of the Northeast — commercial waste is a municipal exclusive franchise: one designated hauler, set rates, no competitive bid. In an exclusive zone there is no bid to win. The honest move is to tell the property manager that, help them right-size within the franchise hauler rate card and service levels, and not pretend a competitive bid exists. Selling a "switch" that the municipal franchise legally forbids destroys credibility.

**Counter-case 3 — Transparency can lose a price-only buyer.** Putting the full fee stack on one line is the right long-term play, but be honest about the short-term cost. A purely price-driven buyer comparing your transparent all-in number to a competitor low container line with a hidden stack will sometimes pick the competitor — your honest number looks higher. You will lose some of those. The training position is that those accounts churn anyway and are not worth a margin-destroying race; but a rep should go in knowing transparency is a retention strategy, not a win-rate maximizer on price-only RFPs.

**Counter-case 4 — Over-aggressive right-sizing backfires.** Downsizing a container or cutting frequency on the audit average is correct only when the audit captured a representative period. A retail center audited in January will under-state volume for a restaurant that triples in summer, or a store that spikes at holiday. Right-size to the realistic peak, not the four-week mean, or you create overflow, illegal dumping, and pest issues — and the tenant fires you for under-service. When seasonality is material, structure a seasonal frequency adjustment rather than a single fixed downsize.

**Counter-case 5 — Sometimes the incumbent is genuinely right-sized.** Not every incumbent is over-servicing. Some national-hauler reps did the work. If the audit shows the containers and frequency are correct and the fee stack is reasonable, do not invent a problem to justify a switch. Compete on service responsiveness, term flexibility, or the organics program — or acknowledge the incumbent earned the renewal and walk. Manufacturing a fake right-sizing case is the fastest way to lose a property manager trust permanently.

**The synthesis.** The five-stage audit-anchored bid is the right default for multi-tenant retail and restaurant accounts with plausible over-service. It is the wrong tool for tiny single-stream accounts, it is moot in exclusive franchise zones, it costs some price-only RFP wins, it must respect seasonality, and it must not invent problems that are not there. Coach the default and coach the exceptions in the same breath.
`;

// ---- polish addition 4: cross-links (8->9) ----
const crossLinks = `
---

## Related Pulse Library Entries — Cross-Links for the Commercial Services Bid

A waste & recycling hauling bid does not sit alone. The buyer — a property manager or restaurant operator — is also evaluating cleaning, pest control, hood cleaning, and grounds vendors, and the same facility-services-bid discipline carries across all of them. Reps and sales managers should read these companion Pulse entries alongside this training.

**q9697 — How do you start a commercial kitchen exhaust hood cleaning business in 2027?** The closest operational neighbor to a restaurant waste account. Hood cleaning and grease management share the same back-of-house dock, the same NFPA and health-code exposure, and the same restaurant decision-maker. A waste rep who understands the hood-cleaning bid can package grease-trap and organics conversations together and speak the operator full back-of-house language.

**q2139 — How do you start a pest control business in 2027?** Waste and pest control are inseparable in a restaurant or retail center: a contaminated, overflowing, or under-serviced container is the number-one pest harborage. This entry — and its sales-training sibling st0028 (Commercial Pest Control Bid Walk) — shows why right-sizing and contamination prevention in a waste bid is also a pest-prevention argument the operator cares about.

**q9610 — How do you start a commercial cleaning business in 2027?** Commercial cleaning and waste hauling are bid by the same property managers, on the same multi-tenant retail and office properties, often in the same RFP cycle. The audit-anchored, transparent-pricing, no-junk-fee discipline in this training maps directly onto the commercial-cleaning bid — and its sales-training sibling st0029 (Commercial Janitorial Bid Walk) runs a parallel five-stage motion.

**q2110 — How do you start a commercial office cleaning business in 2027?** The Class A office and mixed-use property buyer in this entry is the same property manager who signs the waste agreement. The fee-transparency and evergreen-auto-renew conversations in this training apply identically to the office-cleaning service contract.

**q1956 — How do you start a property management business in 2027?** This entry profiles the buyer on the other side of the waste bid table. Understanding how a property manager is measured — total cost across vendors, tenant satisfaction, compliance exposure — is exactly why the WALK, WEIGH, and SHOW stages land: they speak the property manager scorecard back to her.

Read together, these five entries plus the sales-training siblings st0028 (pest control bid) and st0029 (janitorial bid) form the Pulse facility-services-bid cluster — one consistent discipline for any vendor walking a multi-tenant property owner through a service-contract bid.
`;

const fullAnswer = tldr + core + flow + citations + verified + counterCase + crossLinks;
...
(async () => {
  ...
  rec.answer = fullAnswer;
  rec.quality_score = 9;
  await store.setJSON('answers/' + ID + '.json', rec);

  const idx = await store.get('_index.json', { type: 'json' });
  const e = idx.entries.find(x => x.id === ID);
  if (e) { e.quality_score = 9; }
  await store.setJSON('_index.json', idx);

  console.log('restored. id:', ID, '| qs:', rec.quality_score);
  console.log('answer chars:', fullAnswer.length, '| words:', fullAnswer.split(/\s+/).filter(Boolean).length);
  console.log('has mermaid:', fullAnswer.includes('```mermaid'));
  console.log('polish_history len:', (rec.polish_history || []).length);
