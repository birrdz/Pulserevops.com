...
https.get('https://pulserevops.com/.netlify/functions/pulse-machine-library-list?recent=5000', res => {
  ...
  res.on('end', () => {
    ...
    elig.slice(0, 18).forEach((e, i) => console.log('  idx' + i + ' ' + e.id + ' ts=' + e.ts + ' qs=' + e.quality_score + ' fv=' + e.format_v + ' :: ' + (e.question || e.q || '')));
    const target = elig[12];
    console.log('TARGET idx12 = ' + (target ? target.id + ' :: ' + (target.question || target.q || '') : 'NONE'));
  });
