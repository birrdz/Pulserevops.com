...
https.get('https://pulserevops.com/.netlify/functions/pulse-machine-library-list?recent=5000', res => {
  ...
  res.on('end', () => {
    ...
    fs.writeFileSync('C:/Users/koryj/website/lab/_list_t229_idx17.json', JSON.stringify(j));
    ...
    elig.slice(0, 25).forEach((e, i) => console.log('  idx' + i + ' ' + e.id + ' ts=' + e.ts + ' qs=' + e.quality_score + ' fv=' + e.format_v + ' :: ' + e.question));
    const target = elig[17];
    console.log('TARGET idx17 = ' + (target ? target.id + ' :: ' + target.question : 'NONE'));
    // also dump all eligible ids for cross-link verification
    fs.writeFileSync('C:/Users/koryj/website/lab/_eligible_ids_t229.json', JSON.stringify(j.entries.map(e => ({id:e.id,q:e.question})).filter(e=>/^q\d+$/.test(e.id))));
  });
