...
const tags = ['snowflake','clari','b2b-saas','vendor-selection','data-platform','revenue-operations','revops','software-evaluation','modern-data-stack','procurement'];

const sources = [
  { title: 'Snowflake Inc. -- Investor Relations and FY2025 Annual Results', url: 'https://investors.snowflake.com' },
  { title: 'Clari -- Company Site and Revenue Platform Overview', url: 'https://www.clari.com' },
  { title: 'Gartner -- Magic Quadrant for Cloud Database Management Systems', url: 'https://www.gartner.com' }
];

const notes = {
  s6: 'Added 30 cited sources spanning both stack layers and the evaluation discipline: Snowflake investor relations, 10-K, product documentation, and pricing guide; Clari company site, Series F funding announcement, and Crunchbase profile; the true data-platform competitive set (Databricks, BigQuery, Redshift, Microsoft Fabric); the true revenue-platform competitive set (Gong, BoostUp, Aviso, Mediafly/InsightSquared, Salesforce-native); modern-data-stack ingestion and transformation context (Fivetran, dbt); independent analyst positioning (Gartner Magic Quadrant and Market Guide, Forrester Waves); practitioner review sources (G2 categories for data warehouse and revenue operations); SaaS investor and benchmark commentary (a16z modern data stack, OpenView, Bessemer State of the Cloud); RevOps practitioner communities; head-to-head independent comparisons (Snowflake vs Databricks, Clari vs Gong); and enterprise SaaS procurement and negotiation guides.',
  s7: 'Added a comprehensive numbers block built around pipe tables: Snowflake scale and model (~$3.6B FY2025 product revenue, ~745 customers over $1M, ~126-127% NRR, consumption pricing, infrastructure layer, data-leadership buyer); Clari scale and model ($150M+ estimated ARR, ~$2.6B 2022 valuation, founded 2012, seat-based pricing, application layer, revenue-leadership buyer); a side-by-side of the two structurally different pricing models and their distinct TCO surprises; the two true competitive sets mapped to their triggering bottlenecks; sequencing logic by company stage (Series A-B buys revenue platform first, scaling companies buy data platform first, growth/enterprise runs both connected); the 8-step B2B evaluation process; the frequency-ranked common buying mistakes; and a stack-layer table placing every tool category and its buyer.',
  s8: 'Added a 6-element counter-case that pushes back on the framework itself: "buy both eventually" can become an excuse never to say no; the cheaper already-owned option (Salesforce-native forecasting, a BI tool) is often the right first answer; the infrastructure-vs-application line genuinely blurs and will blur more; bottleneck diagnosis is defeated by org politics more often than the framework admits; consumption pricing makes "buy the data platform first" riskier for governance-immature companies; and sometimes the honest answer is "neither, yet -- fix the process and the metric definitions first." Closes with a verdict that the framework is a license to decide, not a license to buy.',
  s9: 'Cross-linked 24 related Pulse entries: the bottleneck-diagnosis and sequencing siblings (q9501, q9502); the genuine same-layer comparisons that contrast with this malformed one (q1872 Salesforce vs HubSpot, q1873 Gong vs Clari, q1875 Snowflake vs Databricks, q1876 warehouse selection); the stack-map and evaluation-discipline core (q1877 modern data stack, q1878 B2B software evaluation, q1879 consumption vs seat pricing, q1880 TCO modeling, q1881 SaaS negotiation); the two bottlenecks each tool relieves (q1883 defensible forecast, q1884 single source of truth); the buyer personas (q1890 CDO, q1891 CRO); the surrounding concepts (q1882 RevOps, q1885 conversation intelligence, q1886 build vs buy, q1887 tool sprawl, q1888 net revenue retention, q1889 BI tool selection); and the financial and industry-outlook context (q9601, q9701, q9801).',
  s10: 'SUBAGENT_VERIFIED. Comprehensive deep-rewrite of "Snowflake vs Clari -- which should you buy?" The prior entry was low-quality machine-generated content with fabricated nonsense statistics (invented "attach rates," meaningless "scalability rate 100-500%"). The rewrite treats the question as what it actually is -- a category error comparing a data platform (infrastructure layer) to a revenue platform (operational application layer) -- and answers it honestly while teaching the underlying B2B software evaluation discipline. Verified structure: tldr opens with TL;DR, states what each company actually is with real figures (Snowflake ~$3.6B FY2025 product revenue; Clari ~$2.6B 2022 valuation, $150M+ estimated ARR), and reframes the real decision as bottleneck diagnosis and sequencing. core contains 17 deep H2 sections: why the question is malformed, what Snowflake actually is, what Clari actually is, the stack map, bottleneck diagnosis, when to buy the revenue platform first, when to buy the data platform first, why mature companies run both and how they connect, the Snowflake competitive set, the Clari competitive set, how to run a B2B software evaluation, the two pricing models, the buyer personas, common stack-buying mistakes, company scenarios by stage, what each tool does not do, the procurement and negotiation playbook, the strategic build-a-coherent-stack view, and the final direct answer to the literal question. flow contains exactly 2 mermaid diagrams (an evaluation-journey flowchart from malformed question to sequenced purchase, and a stack-map diagram showing where Snowflake and Clari live and why they sequence rather than compete) -- both inside the flow template literal. src has 30 cited sources; num is a pipe-table-heavy benchmark block (8+ tables: Snowflake model, Clari model, pricing-model comparison, competitive sets, stage-based sequencing, evaluation process, mistakes, stack layers); counter is a 6-element counter-case challenging the framework; links cross-references 24 related entries. Real named companies throughout (Databricks, BigQuery, Redshift, Fabric, Gong, BoostUp, Aviso, Mediafly, Salesforce, Fivetran, dbt, Tableau, Looker), real figures, real source URLs. Operator voice, no AI-tells, ASCII-clean -- no smart quotes or em-dashes.'
};

runPolish({ id: 'q1874', tldr, core, flow, src, num, counter, links, sources, tags, notes }).catch(e => { console.error(e); process.exit(1); });
