// Quick audit of q104 against the 6 gold format elements.
...
(async () => {
  ...
  const e = await store.get('answers/q104.json', { type: 'json' });
  if (!e) { console.error('q104 not found'); process.exit(1); }
  const ans = e.answer || '';
  fs.writeFileSync(path.join(__dirname, 'q104_raw.md'), ans, 'utf8');
  ...
  const audit = {
    ...
    elements: {
      ...
      e5_real_companies: {
        profitwell: /ProfitWell/i.test(ans),
        patrick_campbell: /Patrick Campbell/i.test(ans),
        paddle: /Paddle/i.test(ans),
        christian_owens: /Christian Owens/i.test(ans),
        chartmogul: /ChartMogul/i.test(ans),
        recurly: /Recurly|RCLY/i.test(ans),
        stripe_sigma: /Stripe Sigma|Sigma/i.test(ans),
        mixpanel: /Mixpanel/i.test(ans),
        saas_capital: /SaaS Capital/i.test(ans),
        todd_gardner: /Todd Gardner/i.test(ans),
        openview: /OpenView/i.test(ans),
        bessemer: /Bessemer|BVP|State of the Cloud/i.test(ans),
        pacific_crest: /Pacific Crest/i.test(ans),
