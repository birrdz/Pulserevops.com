// Generalized keyless image+link backfill via DuckDuckGo for ANY Top-10 pillar.
// Usage: node _img_backfill_any.js <prefix> <pillar-path>
//   e.g. node _img_backfill_any.js dn /dining
// Targets entries whose id matches ^<prefix>\d+$ and that have a Top-10 (## N.)
// structure with fewer than THRESHOLD images. Strips old @@PRODUCT cards,
// rebuilds with a real DDG image + source link per item, HEAD-validates every
// image, writes back only if NOT worse (never regresses). Rate-limit resilient.
const fs = require('fs');
...
const PILLAR_PATH = process.argv[3] || '';
if (!PREFIX || !PILLAR_PATH) { console.error('Usage: node _img_backfill_any.js <prefix> <pillar-path>'); process.exit(1); }
const SINCE = +(((process.argv.find(a => a.startsWith('--since=')) || '').split('=')[1]) || 0); // only entries with ts>=SINCE (today-and-forward scoping)
const PRE_RE = new RegExp('^' + PREFIX + '\\d+$');
const THRESHOLD = 10;
const CONCURRENCY = 3;
...
(async()=>{
  ...
  const progress = createBatchProgressReporter({
    label: `PULSE ${PREFIX} DDG backfill`,
    total: weak.length,
    interval: 10,
    pillarUrl: `https://pulserevops.com${PILLAR_PATH}`,
  });
  await progress.start(`<p>Keyless image backfill for <b>${PREFIX}</b> — ${weak.length} entries</p>`);
  let fixed=0,improved=0,addedImgs=0,cursor=0;
  const cleared=new Set(); // ids that reached the full image set → clear images_pending in index (advances the progress bar)
    ...
    while(cursor<weak.length){
      ...
      try{
        ...
        if(res.imgs>=cur){
          ...
          if(res.imgs>=THRESHOLD)cleared.add(id);
          fixed++;
          console.log(`  [${fixed}/${weak.length}] ${id} ${cur}->${res.imgs} imgs / ${res.links} links`);
        } else {
          console.log(`  keep ${id} ${cur} (ddg got ${res.imgs})`);
        }
      }catch(err){console.log(`  FAIL ${id}: ${err.message}`); await progress.fail(`${id}: ${err.message}`);}
      await progress.tick(`${id} (${fixed} updated)`);
    }
  }
  await Promise.all(Array.from({length:CONCURRENCY},worker));
  // Clear images_pending on completed index rows so pulse-progress (live, no deploy) advances the image bar.
  if(cleared.size){
    try{
  ...
  console.log(`\n[${PREFIX}] DDG DONE. updated=${fixed} improved=${improved} imagesAdded=${addedImgs}`);
  fs.writeFileSync('C:/Users/koryj/website/_'+PREFIX+'_ddg_result.json',JSON.stringify({prefix:PREFIX,targets:weak.length,fixed,improved,addedImgs},null,1));
  await progress.complete(`<p>${PREFIX} keyless image backfill finished. ${improved} entries improved, ${addedImgs} real images added across ${weak.length} targeted entries.</p>`);
