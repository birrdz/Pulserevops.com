// Direct-write for the schools pillar — sc####.
...
//
// Usage: node _write_sc.js <sc####> "<title>" [topic-slug]
...
if (!ID || !/^sc\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_sc.js <sc####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['school', 'top-10', 'best-of-2027', 'schools'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/schools/${ID}`, indexnow: indexed }));
