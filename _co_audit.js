// Collectibles (co####) deep quality auditor.
// Walks co entries newest-first, re-grades each on the shared gold grader
// (co maps to the electronicreview Top-10 ruleset), and deep-checks the
// contract: 1800+ words, 10 numbered item sections, BOTH 🏆 BEST OVERALL
// and 💎 BEST VALUE pills, FAQ, Sources, no banned phrases. Stops after N
// consecutive clean passes (audit early-stop law).
// Usage: node _co_audit.js [maxClean=12]
const fs = require('fs');
...
(async () => {
  ...
  const cos = (idx.entries || []).filter(e => e && /^co\d+$/.test(e.id)).sort((a, b) => (b.ts || 0) - (a.ts || 0));
  ...
  for (const e of cos) {
    ...
    if (prodSecs < 10) issues.push(`item sections ${prodSecs}<10`);
    scanned++;
    if (issues.length) { fails.push({ id: e.id, q: e.question, issues }); streak = 0; }
    else { clean++; streak++; if (streak >= MAX_CLEAN) break; }
  }
  console.log(`CO AUDIT — scanned ${scanned} / co total ${cos.length} | clean ${clean} | failed ${fails.length} | stopped after ${streak} consecutive clean`);
  ...
  fs.writeFileSync('C:/Users/koryj/_co_audit_findings.json', JSON.stringify(fails, null, 2));
