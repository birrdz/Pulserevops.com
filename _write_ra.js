// Direct-write for the Revenue Architecture pillar — ra####.
// Modeled on _write_er.js. Operator-grade essays on GTM design, pipeline
// math, comp + org architecture, segmentation, motion design.
//
// Usage: node _write_ra.js <ra####> "<title>" [topic-slug]
...
if (!ID || !/^ra\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_ra.js <ra####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['revenue-architecture', 'gtm-design', 'revenue-operations', 'sales-leadership', 'cro'];
...
(async () => {
  ...
  let entry = {
    ...
    polish_history: [
      ...
      { from: prevQs, to: 10, at: now, note: isUpgrade ? 'gold-format upgrade revenue-architecture' : 'direct-write 10/10 revenue-architecture' }
    ]
  };
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    ...
    const preCount = (idx.entries || []).length;
  ...
  if (!writeOk) {
    console.error(`WARN: ${ID} index write may have lost the race after 5 attempts. Per-entry blob is safe.`);
  }
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: postCount, url: `https://pulserevops.com/revenue-architecture/${ID}`, indexnow: indexed }));
