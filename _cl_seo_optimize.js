// SEO-optimize all live cl#### entries: semantic FAQ compete keywords + IndexNow.
// Usage: node _cl_seo_optimize.js [--dry-run]
const fs = require('fs');
...
(async () => {
  ...
  const clRows = (idx.entries || []).filter((e) => e && /^cl\d+$/i.test(e.id));
  clRows.sort((a, b) => parseInt(a.id.slice(2), 10) - parseInt(b.id.slice(2), 10));
  ...
  for (const row of clRows) {
    ...
    if (!DRY && changed) {
      ...
      await store.setJSON(`answers/${id}.json`, {
        ...
        seo_keyword_phrase_count: CL_KEYWORD_PHRASES.length,
    ...
    results.push({
      ...
      url: `https://pulserevops.com/clubs/${id}`,
  ...
  let hubPing = null;
  if (!DRY) hubPing = await pingIndexNowUrlList(CL_STATIC_URLS);
  ...
  const report = {
    ...
    keywordPhrases: CL_KEYWORD_PHRASES.length,
    hubPing,
    hubUrls: CL_STATIC_URLS,
    sample: results[0] || null,
  };
  fs.writeFileSync('C:/Users/koryj/website/_cl_seo_optimize_report.json', JSON.stringify(report, null, 1));
