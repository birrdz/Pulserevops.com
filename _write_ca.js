// Direct-write for the Cars pillar — ca####.
...
//
// Usage: node _write_ca.js <ca####> "<title>" [topic-slug]
...
if (!ID || !/^ca\d+$/.test(ID) || !TITLE) {
  console.error('usage: node _write_ca.js <ca####> "<title>" [topic-slug]');
  process.exit(1);
}
const BODY_PATH = `C:/Users/koryj/${ID}_answer.md`;

const BASE_TAGS = ['car-review', 'top-10', 'vehicle', 'auto', 'best-of-2027'];
...
(async () => {
  ...
  console.log(JSON.stringify({ ok: true, id: ID, upgraded: isUpgrade, prev_qs: prevQs, ts: now, total: idx.entries.length, url: `https://pulserevops.com/cars/${ID}`, indexnow: indexed }));
